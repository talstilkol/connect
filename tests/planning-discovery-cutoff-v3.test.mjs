...
import {
  ...
  readOutputRegistry,
  verifyEnvelope,
} from "../scripts/planning-discovery-cutoff-v3-lib.mjs";
...
test("v3 registry freezes the complete successor output denominator", async () => {
  const registry = await readOutputRegistry();
  assert.equal(registry.version, 3);
  assert.equal(registry.outputPaths.length, 4);
  assert.equal(
    registry.successorOutputRegistry.packageMemberPaths.length,
    23,
  );
  assert.equal(
    registry.successorOutputRegistry.reviewAndAcceptancePaths.length,
    5,
  );
  assert.equal(
    registry.successorOutputRegistry.outputPaths.length,
    28,
  );
  assert.equal(
    new Set([
...
test("v3 cutoff and successor directories are separate", async () => {
  const registry = await readOutputRegistry();
  assert.notEqual(
  ...
  assert.equal(
    registry.successorOutputRegistry.outputPaths.some(
