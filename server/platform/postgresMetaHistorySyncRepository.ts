...
export const postgresMetaHistorySyncSql = Object.freeze({
  ...
  lock: `SELECT waba_id AS "wabaId", phone_number_id AS "phoneNumberId", connection_version AS "connectionVersion",
    ...
    FROM meta_history_sync_sessions WHERE tenant_id = $1 AND connection_version = $2 FOR UPDATE`,
  receipt: `INSERT INTO meta_history_sync_events (tenant_id, event_digest, kind, connection_version) VALUES ($1, $2, $3, $4)
  ...
  update: `UPDATE meta_history_sync_sessions SET sharing_state = $2, max_progress = $3, has_conflict = $4
    WHERE tenant_id = $1 AND connection_version = $5 RETURNING tenant_id AS "tenantId"`,
  chunk: `SELECT content_digest AS digest, conflicted FROM meta_history_sync_chunks WHERE tenant_id = $1 AND phase = $2 AND chunk_order = $3 AND connection_version = $4`,
  media: `SELECT content_digest AS digest, conflicted FROM meta_history_sync_media WHERE tenant_id = $1 AND provider_message_id = $2 AND connection_version = $3`,
  insertChunk: `INSERT INTO meta_history_sync_chunks (tenant_id, phase, chunk_order, progress, content_digest, payload, connection_version)
    VALUES ($1, $2, $3, $4, $5, $6::jsonb, $7) RETURNING content_digest AS digest`,
  insertMedia: `INSERT INTO meta_history_sync_media (tenant_id, provider_message_id, content_digest, payload, connection_version)
    VALUES ($1, $2, $3, $4::jsonb, $5) RETURNING content_digest AS digest`,
  conflictChunk: `UPDATE meta_history_sync_chunks SET payload = NULL, conflicted = TRUE WHERE tenant_id = $1 AND phase = $2 AND chunk_order = $3 AND connection_version = $4
    RETURNING content_digest AS digest`,
  conflictMedia: `UPDATE meta_history_sync_media SET payload = NULL, conflicted = TRUE WHERE tenant_id = $1 AND provider_message_id = $2 AND connection_version = $3
    RETURNING content_digest AS digest`,
  clearChunks: `UPDATE meta_history_sync_chunks SET payload = NULL WHERE tenant_id = $1 AND connection_version = $2 AND payload IS NOT NULL`,
  clearMedia: `UPDATE meta_history_sync_media SET payload = NULL WHERE tenant_id = $1 AND connection_version = $2 AND payload IS NOT NULL`,
  ...
  return Object.freeze({
    async record(rawScope: MetaHistoryScope, rawItem: MetaHistoryItem) {
      ...
      return transactions.transaction({ isolationLevel: "read-committed" }, async (tx) => {
        ...
        return captureMetaHistoryInTransaction(tx, scope, item);
      });
    },
  });
}

// Shared only by webhook capture and the approved-attribution importer. The
// caller must hold the tenant attribution and authorized connection locks.
export async function captureMetaHistoryInTransaction(tx: PostgresTransaction, rawScope: MetaHistoryScope, rawItem: MetaHistoryItem) {
  const { scope, item } = normalizeMetaHistorySync(rawScope, rawItem);
  ...
  const receipt = await one(tx, postgresMetaHistorySyncSql.receipt, [scope.tenantId, digest, item.kind, scope.connectionVersion]);
  ...
  if (item.kind === "declined") {
    ...
    await tx.query(postgresMetaHistorySyncSql.clearChunks, [scope.tenantId, scope.connectionVersion]);
    await tx.query(postgresMetaHistorySyncSql.clearMedia, [scope.tenantId, scope.connectionVersion]);
    outcome = "declined";
  } else {
    ...
    const previous = await one(tx, chunk ? postgresMetaHistorySyncSql.chunk : postgresMetaHistorySyncSql.media, [...identity, scope.connectionVersion]);
    ...
    if (previous === null) {
      key(await one(tx, chunk ? postgresMetaHistorySyncSql.insertChunk : postgresMetaHistorySyncSql.insertMedia,
        chunk ? [...identity, item.progress, digest, payload, scope.connectionVersion] : [...identity, digest, payload, scope.connectionVersion]), "digest", digest);
    } else {
      ...
      if (stored.digest !== digest) {
        key(await one(tx, chunk ? postgresMetaHistorySyncSql.conflictChunk : postgresMetaHistorySyncSql.conflictMedia, [...identity, scope.connectionVersion]), "digest", stored.digest);
        hasConflict = true;
  ...
  key(await one(tx, postgresMetaHistorySyncSql.update, [scope.tenantId, sharingState, maxProgress, hasConflict, scope.connectionVersion]), "tenantId", scope.tenantId);
