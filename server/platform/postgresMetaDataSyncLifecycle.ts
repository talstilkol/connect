...
export const postgresMetaDataSyncLifecycleSql = Object.freeze({
  ...
  jobView: `SELECT job.status,clock_timestamp()>=job.started_at+INTERVAL '24 hours' AS expired,
    ...
    WHERE job.tenant_id=$1 AND tenant.status IN ('active','trial','payment_failed')
      AND NOT EXISTS (SELECT 1 FROM meta_data_sync_onboardings WHERE tenant_id=job.tenant_id AND signup_launch_id=job.launch_id)
      AND NOT EXISTS (SELECT 1 FROM meta_data_sync_onboardings WHERE tenant_id=job.tenant_id AND signup_launch_id>job.launch_id)
    ORDER BY job.launch_id DESC LIMIT 1`,
  // Selection only; all eligibility is recomputed after locking below.
  candidate: `SELECT DISTINCT request.tenant_id AS "tenantId",request.started_at AS "startedAt" FROM meta_data_sync_requests AS request
    ...
    LEFT JOIN meta_data_sync_requests AS contacts ON contacts.tenant_id = request.tenant_id AND contacts.started_at=request.started_at AND contacts.sync_type = 'smb_app_state_sync'
    ...
    ORDER BY request.tenant_id,request.started_at LIMIT 1`,
  tenant: `SELECT status FROM tenants WHERE id = $1 FOR NO KEY UPDATE`,
  requests: `SELECT sync_type AS "syncType", status, waba_id AS "wabaId", phone_number_id AS "phoneNumberId",
    ...
    WHERE tenant_id = $1 AND started_at=$2::timestamptz ORDER BY sync_type FOR UPDATE`,
  view: `SELECT onboarding.started_at AS "startedAt", onboarding.baseline_connection_version AS "baselineVersion",
    ...
    LEFT JOIN meta_data_sync_requests AS history ON history.tenant_id = tenant.id AND history.started_at=onboarding.started_at AND history.sync_type = 'history'
    ...
    WHERE tenant.id = $1 AND tenant.status IN ('active', 'trial', 'payment_failed')
      AND NOT EXISTS (SELECT 1 FROM meta_coexistence_sync_jobs job WHERE job.tenant_id=tenant.id AND job.started_at>onboarding.started_at)`,
});
...
export function createPostgresMetaDataSyncLifecycle(dependencies: Readonly<{
  ...
  return Object.freeze({
    async readView(rawTenantId: number, expected: MetaConnectionRecord | null): Promise<Readonly<MetaDataSyncView> | null> {
      ...
      else if (row.sharingState === "unattributed" && row.history === "accepted") stage = "import-review-required";
      ...
      const hide = prepared || ["connection-changed", "sharing-declined", "conflicted", "import-review-required"].includes(stage);
    ...
    async reconcileNext(): Promise<"idle" | "reconciled"> {
      return dependencies.transactions.transaction({ isolationLevel: "read-committed" }, async (tx) => {
        ...
        const selected = requireExactPostgresRow(candidate, ['tenantId','startedAt']);
        const tenantId = parsePostgresPositiveInteger(selected.tenantId), startedAt = parsePostgresTimestamp(selected.startedAt);
        // Match claim's tenant-first order and lock only this cycle's requests.
        // Concurrent accepted contacts evidence is observed before cancellation.
        const tenant = requireExactPostgresRow(await one(postgresMetaDataSyncLifecycleSql.tenant, [tenantId]), ["status"]);
        const requests = requirePostgresRows(await tx.query(postgresMetaDataSyncLifecycleSql.requests, [tenantId,startedAt]),2).map((row) =>
        ...
        for (const request of requests) {
          ...
          const contacts = requests.find((other) => other.syncType === "smb_app_state_sync" && parsePostgresTimestamp(other.startedAt) === start);
          ...
          const stopped = await one(postgresMetaDataSyncSql.stop, [tenantId, String(request.syncType), status,start]);
