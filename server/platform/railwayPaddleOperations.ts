...
export function createRailwayPaddleOperations(dependencies: {
  ...
  billing: Readonly<{ journal: PostgresPaddleRepository; plan: PaddlePlan; clientToken: string; customerPortalUrl: string }> | null;
  ...
  return ["billing.current.read", PADDLE_CHECKOUT_OPERATION].map(id => ({ id, requestKind: id === PADDLE_CHECKOUT_OPERATION ? "mutation" : "query",
    async execute(context, payload, request) {
      try {
        ...
        if ((mutation ? Object.keys(payload).join(",") !== "expectedAttempt" || !isPaddleCheckoutAttempt(payload.expectedAttempt) : Object.keys(payload).length !== 0) || request.operation !== id || request.requestKind !== (mutation ? "mutation" : "query") ||
          request.idempotencyKey !== (mutation ? await deriveRailwayApiDeterministicIdempotencyKey(id, payload) : null)) throw new RailwayApiDispatchError("INVALID_REQUEST");
        ...
        if (mutation) {
          ...
          await dependencies.billing.journal.enqueue(session, dependencies.billing.plan, payload.expectedAttempt as number);
        }
        return { ...await dependencies.billing.journal.read(session, dependencies.billing.plan.environment), clientToken: dependencies.billing.clientToken, customerPortalUrl: dependencies.billing.customerPortalUrl };
      } catch (error) {
