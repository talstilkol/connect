...
export interface PaddleConfiguration extends PaddlePlan { readonly apiKey: string; readonly webhookSecret: string; readonly clientToken: string; }
export function readPaddleEnvironment(): PaddleRuntimeEnvironment {
  return { PADDLE_ENABLED: process.env.PADDLE_ENABLED, PADDLE_ENVIRONMENT: process.env.PADDLE_ENVIRONMENT,
...
export function requirePaddleConfiguration(env: PaddleRuntimeEnvironment): PaddleConfiguration | null {
  ...
  try {
    ...
    return Object.freeze({ environment: env.PADDLE_ENVIRONMENT as "sandbox" | "production", apiKey: env.PADDLE_API_KEY!, clientToken: env.PADDLE_CLIENT_TOKEN!, webhookSecret: env.PADDLE_WEBHOOK_SECRET!,
