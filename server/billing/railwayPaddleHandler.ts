...
import { RAILWAY_API_CONTRACT_VERSION, type RailwayApiJsonObject } from "../platform/railwayApiContract.ts";
...
export function createRailwayPaddleHandler(dependencies: RailwayAiAgentHandlerDependencies) {
  async function call(mutation: boolean, expectedAttempt?: number): Promise<PaddleBillingResult> {
    try {
      if (mutation && !isPaddleCheckoutAttempt(expectedAttempt)) return { status: "server-error" };
      const payload: RailwayApiJsonObject = mutation ? { expectedAttempt: expectedAttempt! } : {};
      ...
      const response = await client.call({ contractVersion: RAILWAY_API_CONTRACT_VERSION, operation, requestKind: mutation ? "mutation" : "query",
        idempotencyKey: mutation ? await deriveRailwayApiDeterministicIdempotencyKey(operation, payload) : null, payload });
  ...
  return { read: () => call(false), createCheckout: (expectedAttempt: number) => call(true, expectedAttempt) };
}
