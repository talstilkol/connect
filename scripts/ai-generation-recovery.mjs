...
export async function runAiGenerationRecovery(args,env=process.env){
  ...
  const confirmations={'usage-confirmed':'CONFIRM_PROVIDER_FINAL_USAGE','no-charge-confirmed':'CONFIRM_PROVIDER_TERMINAL_NO_CHARGE'};
  ...
  try{const service=createAiGenerationRecovery(createNodePostgresTransactionManager(pool));
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){
  try{process.stdout.write(JSON.stringify(await runAiGenerationRecovery(process.argv.slice(2)))+'\n');}
  catch{process.stderr.write('AI_RECOVERY_BLOCKED: verify private evidence, authorization, exact request, final usage and proposal freshness.\n');process.exitCode=1;}
}
