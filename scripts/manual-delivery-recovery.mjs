...
export async function runManualDeliveryRecovery(args,env=process.env){
  ...
  const url=new URL(env.MANUAL_RECOVERY_DATABASE_URL??'');
  ...
  try{const service=createManualDeliveryRecovery(createNodePostgresTransactionManager(pool));
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){
  try{process.stdout.write(JSON.stringify(await runManualDeliveryRecovery(process.argv.slice(2)))+'\n');}
  catch{process.stderr.write('MANUAL_DELIVERY_RECOVERY_BLOCKED: verify private evidence, authorization, original message identity and proposal freshness.\n');process.exitCode=1;}
}
