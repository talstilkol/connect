...
} from "./planning-discovery-cutoff-v3-lib.mjs";

const toolchainPaths = Object.freeze([
  "scripts/planning-discovery-cutoff-v3-lib.mjs",
  "scripts/create-planning-discovery-cutoff-v3.mjs",
  "scripts/verify-planning-discovery-cutoff-v3.mjs",
  "docs/planning/discovery-cutoff-output-path-registry-v3-2026-08-30.json",
  ...
  "docs/planning/discovery-cutoff-v3-tool-contract-2026-08-30.md",
  ...
  "tests/planning-discovery-cutoff-v3.test.mjs",
  "tests/secret-hygiene.test.mjs",
...
function parseArguments() {
  ...
  if (
    ...
    throw new Error(
      "usage: node scripts/create-planning-discovery-cutoff-v3.mjs --observed-at YYYY-MM-DDTHH:mm:ssZ",
    );
  }
...
function emitAddFilePatch(files) {
...
function sourceCandidatePayload({
  ...
  return Object.freeze({
    schema:
      "CONNECT-DISCOVERY-SOURCE-CANDIDATES-PAYLOAD-V3",
    observedAt,
    observedHead,
    authority:
...
async function main() {
  ...
  for (
    const path of
    registry.successorOutputRegistry.outputPaths
  ) {
    await assertAbsent(path);
  }
  await assertAbsent(
    registry.successorOutputRegistry.outputDirectory,
  );
  ...
  const receiptPayload = Object.freeze({
    schema:
      "CONNECT-DISCOVERY-CUTOFF-RECEIPT-PAYLOAD-V3",
    artifactId:
      "CONNECT-DISCOVERY-CUTOFF-CANDIDATE-V3-2026-08-30",
    status:
    ...
    declaredOutputRegistry: Object.freeze({
      ...
      successorOutputRegistry: Object.freeze({
        path:
          registry.successorOutputRegistry.registryPath,
        sha256:
          registry.successorOutputRegistry.registrySha256,
        artifactId:
          registry.successorOutputRegistry.artifactId,
        outputDirectory:
          registry.successorOutputRegistry.outputDirectory,
        packageMemberPaths:
          registry.successorOutputRegistry.packageMemberPaths,
        reviewAndAcceptancePaths:
          registry.successorOutputRegistry.reviewAndAcceptancePaths,
        outputPaths:
          registry.successorOutputRegistry.outputPaths,
        allOutputsAbsentAtCutoff: true,
      }),
  ...
  const receiptEnvelope = createEnvelope({
    schema:
      "CONNECT-DISCOVERY-CUTOFF-RECEIPT-ENVELOPE-V3",
    domain:
      "CONNECT.DISCOVERY-CUTOFF-RECEIPT.V3",
    payload: receiptPayload,
  });
  const candidatesEnvelope = createEnvelope({
    schema:
      "CONNECT-DISCOVERY-SOURCE-CANDIDATES-ENVELOPE-V3",
    domain:
      "CONNECT.DISCOVERY-SOURCE-CANDIDATES.V3",
  ...
  const manifestPayload = Object.freeze({
    schema:
      "CONNECT-DISCOVERY-CUTOFF-MANIFEST-PAYLOAD-V3",
    observedHead: head,
    ...
    packageContentRootSha256: domainDigest(
      "CONNECT.DISCOVERY-CUTOFF-PACKAGE-CONTENT.V3",
      members,
    ),
  ...
  const manifestEnvelope = createEnvelope({
    schema:
      "CONNECT-DISCOVERY-CUTOFF-MANIFEST-ENVELOPE-V3",
    domain:
      "CONNECT.DISCOVERY-CUTOFF-MANIFEST.V3",
    payload: manifestPayload,
  });
  const manifestBytes = jsonBytes(
    manifestEnvelope,
  );
  console.log(emitAddFilePatch([
    {
      path: receiptPath,
      bytes: receiptBytes,
    },
