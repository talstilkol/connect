...
דריסת `steps.install.commands` מחליפה גם את פקודות ההעתקה האוטומטיות, ולכן
התצורה מעתיקה במפורש את `package.json` ואת `package-lock.json` לפני ההתקנה.
קבלת הבנייה דורשת תוכנית Railpack ובניית image בפועל; הצלחת בדיקות האפליקציה
או יצירת תוכנית בלבד אינה מוכיחה שקובצי ההתקנה נמצאים בשכבת הבנייה.
