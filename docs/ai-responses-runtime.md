...
3.3 אחזור סורק את המקורות המאושרים בדפי Keyset של עד 100 קטעים, תחת חסם של עד 1,000 קטעים לכל מקור. הוא מאמת שיוך, Hash ומפתח דטרמיניסטי, מדרג לפי חפיפת מילות שאילתה ושומר את 100 המועמדים הטובים ביותר בין דפים. מתוכם נבחרים עד 20 קטעים/64 KiB. זהו אחזור לקסיקלי מוגבל, שאינו מוכיח Recall מלא או הבנה סמנטית. Tenant ID וכתובת אחסון אינם נשלחים למודל.
...
4.2 מסלולי [Upload/Scan/Extraction](knowledge-ingestion-runtime.md), אישור אדם ו־[Sender עם גבולות WhatsApp](ai-reply-delivery-runtime.md) כבר ממומשים בקוד המקומי. נדרשים חיבורם לספקים, קבלת ביטול גישה ו־Evals בחשבון אמיתי. קיום המתאמים והבדיקות המקומיות אינו קבלת ספק; [מדריך מוכנות AI](ai-publication-readiness.md) מפרט את תנאי ההפעלה.
...
5.1 ספירת הקלט כוללת את אותם model, input, instructions, tools, tool_choice, truncation ו־text.format של בקשת היצירה. הספירה אינה אומדן לפי אורך תווים. תקלה זמנית בספירה דוחה את העבודה בלי ליצור Claim או קריאת Generation. ספירה תקינה שחורגת מ־OPENAI_MAX_INPUT_TOKENS מחזירה policy-violation ומפעילה Handoff קיים עם Audit, בלי Claim או Generation; אותו קלט אינו נכנס ל־Retry עקב החריגה. הרשאה מוקדמת אינה שריון כספי.
...
## 6. R220 — בירור עלות לא ודאית

נוסף כלי פרטי לרישום Usage סופי או היעדר חיוב מאומת, בלי למחוק את הראיות המקוריות ובלי לחזור על Generation. ראיות מאוחרות נשמרות וסתירה פותחת בירור חדש. [הפעלה והרשאות](uncertain-outcome-recovery.md).
