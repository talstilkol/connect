...
עדכון ממוקד Coexistence: 2026-09-09. יתר סעיפי המסמך לא אומתו מחדש בתאריך זה.
...
2.3.1 אומת שוב ב־09.09.2026 מול [Meta — Onboard WhatsApp Business app users](https://developers.facebook.com/documentation/business-messaging/whatsapp/embedded-signup/onboarding-business-app-users/), דף המציג עדכון 26.06.2026. זו מגבלת throughput מתועדת, לא quota יומית של חשבון או הוכחה שמספר מסוים זכאי לחיבור. אין שינוי לערכי שליחה ב־runtime בעקבות קריאת המסמך.

2.3.2 במסלול Coexistence הודעה שנשלחה מהאפליקציה העסקית אינה פותחת או מאריכה חלון שירות של Cloud API. היסטוריה מסונכרנת ו־smb_message_echoes לא ייחשבו ב־Connect כ־inbound חדש לצורך Bot/חלון שירות. זהו תנאי מימוש בשלב 4.6–4.7 של [תוכנית השחרור](planning/launch-master-plan-2026-09-09.md), לא יכולת שהופעלה כבר.
...
## 18. עדכון Coexistence והיסטוריה — 09.09.2026

18.1 בעלות מחקר ופיתוח: Tal (טל). מקור: [Meta — history webhook reference](https://developers.facebook.com/documentation/business-messaging/whatsapp/webhooks/reference/history/), נקרא מחדש ב־09.09.2026. התיעוד מתאר היסטוריה של עד 180 ימים ומזהי מדיה באירועים נפרדים עבור 14 הימים שלפני Onboarding. אלה מאפייני הסנכרון המתועדים, לא מדיניות Retention של Connect ולא פתיחת חלון שירות.

18.2 קוד `2593109` מציין שהעסק דחה שיתוף היסטוריה. אין לפרשו כ־Rate limit או לבצע בגללו POST אוטומטי נוסף. Connect שומר סירוב סופי לאותו ניסיון ומנקה תוכן שנקלט באזור ההיסטוריה, בלי לשנות הסכמות דיוור. סירוב שמגיע לאחר POST שנדחה יכול עדיין לנקות מידע שנקלט קודם לכן, תחת אותם נכסים וחיבור תקף.

18.3 גבול Payload מקומי ל־Railway הוא 2MiB לפני Base64, ונבדק ב־HTTP/Publisher/BullMQ/Consumer. מסלול Cloudflare נשאר עם 120,000 bytes כברירת מחדל. אין מקור Meta שנקרא המבטיח שאצוות history יוגבלו לגודל זה; התאמה לאצוות אמיתיות, זמן עיבוד, Redis retention וצריכת זיכרון מחייבים Staging. הגבול אינו משנה מכסת הודעות, Throughput או Retry-After של ספק.

18.4 התקדמות history נשמרת בנפרד מהשלמה מקומית. תוכן היסטורי אינו עובר למסלול inbound חי ואינו פותח Customer service window. פירוט מימוש, בדיקות ותכולה שנותרה בסעיפים 25–26 של [Master Plan](planning/launch-master-plan-2026-09-09.md).


18.5 ההקרנה ל־Inbox משתמשת בהפניות היסטוריות נפרדות; messages החיה נשארת מקור חלון השירות והאוטומציה. עיבוד ההקרנה מוגבל מקומית ל־100 הודעות בטרנזקציה ולעד 10 מקטעים בהרצת תחזוקת Worker. אלה גבולות עבודה של Connect, ללא שינוי ב־Throughput או מכסת Meta, וללא POST נוסף לספק.


18.6 תחזוקת הסנכרון סוגרת בקשות prepared שפג חלון ההפעלה שלהן לפי השעון השמרני של Connect מתחילת הניסיון. בקשות שכבר נשלחו ותוצאותיהן אינן מאופסות. כשל סופי בבקשת אנשי הקשר מבטל בקשת היסטוריה שטרם נשלחה. הגבול המקומי הוא עד 10 Tenants בכל הרצת תחזוקה, ללא פעולה אצל הספק. המקור לכלל החד־פעמי ולצורך ב־offboarding והרשמה מחדש: [Meta — Coexistence](https://developers.facebook.com/documentation/business-messaging/whatsapp/embedded-signup/onboarding-business-app-users/), נקרא מחדש ב־09.09.2026. אין שינוי במכסות Meta או ב־Retry-After.

18.7 הבהרת שעון, 09.09.2026: Meta מתאר חלון של 24 שעות לאחר Onboarding. Connect מתחיל לספור מוקדם יותר, מזמן ניסיון ההרשמה שנשמר בשרת, כמדיניות שמרנית. בהכנה הקשורה לקבלת הרשמה שהצליחה, הזמן המקורי מועתק ואינו מתחדש בעת השלמה או ניסיון חוזר. זהו חיזוק שיוך במסד, ללא שינוי באורך החלון, בתקרות ספק או במכסות שליחה; המסלול טרם מפעיל את בקשות Coexistence.


18.8 מדיניות המשך מקומית, 09.09.2026, בבעלות Tal: משימות Coexistence נבדקות
בתחזוקת ה־Worker עד **10 משימות להרצה**, עם Lease של **2 דקות** והמתנה
של **לפחות דקה** לפני בדיקה חוזרת. אלה ערכי תפעול של Connect ולא מכסות Meta.
ה־Worker אינו מאריך את חלון ההפעלה המקורי ואינו חוזר על POST שכבר נצרך או
שתוצאתו אינה ודאית. חזרה לתור מאפשרת קריאת מצב או GET שנכשל לפני Claim.
כבר ניתן להפעיל את התזמור הפנימי עם משימה שמורה; נתיב Business App הציבורי
עדיין חסום. סעיף זה מעדכן את מצב המימוש שתואר בסוף 18.7, ללא שינוי במגבלות
הספק או בבעלות המחקר. [מימוש ואימות](planning/launch-master-plan-2026-09-09.md).

18.9 מדיניות קריאת מצב מקומית, 09.09.2026, בבעלות Tal: חלון החיבור מרענן
את מצב הסנכרון עד **12 פעמים** במרווחים של **5 שניות**, רק כשהחלון פתוח והדף
גלוי. הדף אינו מתקדם במונה הרענונים בזמן שהוא מוסתר. בסיום התקציב נשאר רענון
ידני; סגירת החלון או יציאה ממצב סנכרון פעיל עוצרות את המחזור. אלה קריאות מצב
של Connect, ללא POST לספק וללא שינוי במכסות Meta או בהיתר Retry.
ה־API והממשק מחוברים כעת למסלול Business App שמושבת כברירת מחדל ומותר רק
כאשר מתג controlled-pilot מוגדר בשרת. המתג לא הופעל בסביבה חיה בסבב זה.
סעיף זה מעדכן את מצב המימוש בסוף 18.8. ניסיון נוסף לפתוח את דף Coexistence
הרשמי ב־09.09.2026 החזיר 429; לא הוסקו ממנו מגבלות ספק חדשות. המחקר הרשמי
הקודם נותר המקור המתועד. [מימוש וראיות](planning/launch-master-plan-2026-09-09.md).

18.10 שיוך מדיה היסטורית, 09.09.2026, בבעלות Tal: תחזוקת ה־Worker מבצעת
עד **100 ניסיונות שיוך מקומיים להרצה** לאחר הקרנת ההודעות ל־Inbox. השיוך
כותב הפניות למקורות שמורים בלבד; הוא אינו מוריד קובץ ואינו פונה ל־Meta.
הגבול אינו מכסת ספק, אינו משנה את חלון הסנכרון ואינו מוכיח שהייבוא הושלם.
תיעוד [Meta הרשמי ב־Postman — Retrieve Media URL](https://www.postman.com/meta/whatsapp-business-platform/request/fpj02x0/retrieve-media-url)
נקרא ב־09.09.2026: הפרמטר phone_number_id מאפשר לבדוק שהמדיה שייכת למספר,
וכתובת ההורדה תקפה לחמש דקות ודורשת Token. הוחלט שמימוש ההורדה בהמשך
ישתמש בבדיקת הבעלות ובכתובת טרייה, ללא כתובת ספק בדפדפן. אלה דרישות המשך;
אין בסבב זה GET של מדיה או שינוי במכסות השליחה. פתיחה נוספת של דף history
הרשמי החזירה 429; תבנית האירוע נבדקה מול העותק הרשמי שנשמר מוקדם יותר באותו יום.


18.11 הורדת מדיה פנימית, 09.09.2026, בבעלות Tal: שתי בקשות GET לכל ניסיון
תקין, תחילה Graph עם phone_number_id ולאחריו כתובת הורדה טרייה. מדיניות
משאבים מקומית: עד **100 MiB לקובץ**, עד **64 KiB לתשובת מטא־נתונים**,
**30 שניות לכל GET** כברירת מחדל והורדה פעילה **אחת בכל מופע שירות**.
המועד האחרון חל גם על קריאת הגוף; שני GET ובדיקות המסד אינם מוגבלים יחד
ל־30 שניות. מגבלת המופע אינה מכסה מבוזרת בין Workers. אין Retry אוטומטי,
גם בעקבות 429 או 5xx, ואין שמירת כתובת ספק או Token בתוצאה.
אלה גבולות Connect, לא מגבלות Media או מכסות שליחה של Meta. השימוש בכתובת
טרייה ובבדיקת בעלות נשען על מקור Meta ב־Postman שבסעיף 18.10; ה־Allowlist
המצומצמת של כתובת ההורדה היא בחירה מקומית שדורשת אימות ב־Staging.
סעיף זה מעדכן את מצב המימוש בסוף 18.10: ההורדה הפנימית נבדקה עם PostgreSQL
ו־Vault אמיתיים ו־fetch בדיקה. אחסון פרטי, סריקה, עבודות הורדה עמידות וחיבורן
ל־Worker עדיין חסרים; לא נשלחו GET לחשבון Meta חי ולא השתנו מכסות הספק.
[מימוש ואימות](planning/launch-master-plan-2026-09-09.md).


18.12 אחסון מדיה בהסגר, 10.09.2026, בבעלות Tal: מתאם S3 הפנימי מבצע
שישה GET של תצורת Bucket לכל ניסיון כתיבה, במקביל, ואחריהם PUT מותנה אחד
רק אם התצורה וההרשאה תקינות. עד 100 MiB לקובץ והעלאה פעילה אחת במופע
שירות/מתאם; 30 שניות לכל בקשת S3 כברירת מחדל, לא לכל רצף העבודה.
אין Retry ב־SDK או בשירות. תוצאת 412 או תוצאה לא ודאית אינן אישור הצלחה.
לצורך מניעת שינוי Bytes בזמן המתנה נוצר עותק נוסף, כך ששני Buffers יכולים
להחזיק יחד עד 200 MiB, נוסף על תקורת SDK וחישוב Hash. יש למדוד זיכרון
ב־Staging לפני הפעלה. אלה גבולות משאבים של Connect בלבד; אין שינוי במכסות
Meta, בחלון הסנכרון או בהיתר שליחה. Runtime זה טרם הופעל אוטומטית ואין
בקשות S3 או Meta חיות בסבב זה. [מימוש, תצורה וגבולות](s3-meta-media-quarantine.md).


18.13 יומן העלאות עמיד, 10.09.2026, בבעלות Tal: Lease מקומי של **חמש
דקות** וגרסת Claim לכל מקור מדיה; תפיסה חוזרת מותרת רק לפני dispatching.
אחרי רישום שיגור אין PUT אוטומטי נוסף, גם אם פג הזמן או אבד אישור Commit.
חזרה לאישור שכבר נשמר מבצעת **אפס בקשות Meta ואפס בקשות S3**, לאחר אימות
הרשאה ויעד. לפני האחסון הראשון עדיין מתבצעת הורדת Meta מאומתת, ואין עדיין
תזמון הורדות אוטומטי ב־Worker. תקרות הגודל, הזיכרון וזמן הבקשה שבסעיפים
18.11–18.12 נשארות ללא שינוי. זהו מנגנון תיאום של Connect, לא מכסת ספק
או שינוי בחלון הסנכרון. [יומן, תרחישי כשל ובדיקות](planning/launch-master-plan-2026-09-09.md).


18.14 בירור מדיה וסריקה, 10.09.2026, בבעלות Tal: תהליך פנימי בקריאה בלבד,
עם ניסיון פעיל אחד בכל מופע ו־**30 שניות לכל בקשת S3** כברירת מחדל.
אחרי שש בדיקות תצורה, אישור חסר מחייב לכל היותר ListObjectVersions אחד
עם **MaxKeys=2**; אין Pagination או Retry. בהמשך נקראות תגיות לגרסה
מפורשת; רק תגית נקייה מאפשרת HEAD עם checksum ואחריו בדיקת תגית נוספת.
עד **עשר בקשות S3 לניסיון** במצב זה, או **תשע** כאשר VersionId כבר שמור.
תוצאה חסרה או חוסמת נעצרת מוקדם יותר. אין GET של תוכן קובץ, PUT או בקשת
Meta. גבולות אלה הם מדיניות Connect בלבד, ללא שינוי במכסות Meta או AWS.
תזמון בדיקות חוזרות והמתנה ביניהן ב־Worker עדיין נדרשים; קריאה חוזרת ידנית
אינה מכסה מבוזרת. [חוזה הבירור וההרשאות](s3-meta-media-quarantine.md).

18.15 תזמון מדיה, 10.09.2026, בבעלות Tal: נוסף Worker עם upload או inspect
בתהליכים נפרדים ובבחירה מפורשת בלבד. היעדר META_MEDIA_WORKER_MODE משאיר
אותו כבוי. גבולות Connect: **100 גילויים וחמש עבודות בסדרה לסבב**, עבודה
פעילה אחת למופע, **דקה** המתנה לאחר סבב, **Lease של עשר דקות**, עד **שלושה
ניסיונות upload או 12 ניסיונות inspect**. השהיית Retry מתחילה בדקה,
מוכפלת ונחסמת ב־15 דקות. גם ניסיון שלא הגיע לספק נספר. אלו אינם גבולות
Meta/AWS או מכסה מבוזרת לכלל הפריסה. הרשאות נבדקות מחדש; שיגור S3 שכבר
נרשם אינו מתאפס ואינו נשלח שוב. מיצוי ניסיונות מחייב טיפול תפעולי.
סעיף זה מעדכן את דרישת תזמון הרקע בסעיפים 18.13–18.14; מגבלות הקובץ,
הבקשות וחלונות הסנכרון נשארות כפי שתועדו. [מדיניות והפעלה](s3-meta-media-quarantine.md).

18.16 אבחון משימות מדיה, 10.09.2026, בבעלות Tal: נוספה קריאת בעלים
מוגבלת ל־**50 משימות בעמוד ועוד רשומת look-ahead פנימית**. הרשאה נבדקת
בכל קריאה; אין ספירה כוללת, שינוי pageSize או Polling אוטומטי. רענון
ודפדוף אינם מפעילים Worker ואינם צורכים בקשות Meta/AWS. גבולות הניסיונות
והתזמון של 18.15 נשארו ללא שינוי; המסך אינו מאפס מונה או יומן שיגור.
זו מגבלת קריאה מקומית של Connect, לא מכסת ספק. [חוזה האבחון](s3-meta-media-quarantine.md).


18.17 קריאת קובץ פנימית, 10.09.2026, בבעלות Tal: עד **100 MiB**, קריאה
פעילה אחת למופע שירות ועד **תשע בקשות S3** למסלול תקין — שש בדיקות Bucket,
שתי קריאות תגית ו־GET גוף אחד. ברירת המחדל היא **30 שניות לבקשה**, כולל
העברת הגוף ואימות Hash בבקשת GET. אין Retry, Range או בקשת Meta. אלה גבולות
Connect; אין מהם מכסה מבוזרת, מגבלת זיכרון כוללת או מכסת AWS/Meta רשמית.
נדרשים מדידת זיכרון, הגבלת תעבורה, ביטול ו־Backpressure לפני חשיפת נתיב
הורדה. ה־Runtime כרגע פנימי ואינו משנה זמינות קבצים ב־Inbox.
[חוזה הקריאה](s3-meta-media-quarantine.md).

18.18 הורדת HTTP מאומתת, 10.09.2026, בבעלות Tal: לפיילוט Connect נקבעו
**10 ניסיונות מורשים בכל 60 שניות למשתמש ול־Tenant**. PostgreSQL סופר רישומי
read-admitted באמצעות השעון שלו, לאחר הרשאה ונעילת מקור משותפת למופעי API.
כשל כתיבת Audit חוסם S3; ניסיון שהתקבל ואז נכשל או בוטל נשאר נספר. 429 מחזיר
Retry-After:60. זו מכסת Connect חדשה ואינה ערך Meta או AWS.

18.18.1 נשמרות תקרת 100 MiB, עד תשע בקשות S3 תקינות ו־30 שניות לבקשת S3.
נתיב HTTP מוסיף **בקשה אחת פעילה למופע**, ללא תור, ו־**120 שניות** לכל הבקשה,
כולל אימות ומסירה. בקשה מקבילה מוחזרת 503 עם Retry-After:5 לפני עבודה יקרה.
מקטעי Node הם עד 64 KiB, עם המתנה לכתיבה. Timeout אינו משחרר את המקום כל עוד
תלות בפעולה עדיין לא הסתיימה. יש למדוד זיכרון, קצב וריבוי מופעים ב־Staging
ולהגדיר הגבלת תעבורה בפלטפורמה לפני הרחבת הפיילוט. אין Polling/Retry אוטומטי
בדפדפן. [חוזה HTTP](vercel-railway-api-contract.md), [תשתית](s3-meta-media-quarantine.md).

18.19 **הארכה מפורשת לבירור מדיה, מדיניות Connect של Tal, 10.09.2026:** בסיס
12 הניסיונות האוטומטיים של inspect נשאר ללא שינוי. אחרי מיצוי בלבד, בעלים יכול
להעניק **ניסיון אחד בכל בקשה**, עד **שלוש בקשות מצטברות למשימה**. התקרה היא
12, 13, 14 או 15 לפי מספר הבקשות שכבר נשמרו; אין איפוס ואין Retry אוטומטי מהדפדפן.
Idempotency וגרסת משימה מונעים חריגה גם בבקשות מקבילות או אחרי אובדן תשובת Commit.
בקשה, תוספת לתור ו־Audit נשמרים אטומית. upload נשאר עם שלושה ניסיונות ומניעת PUT
חוזר לאחר שיגור. סריקה חוסמת, סתירת גרסאות וביטול אינם מקבלים הארכה.

18.19.1 כל הארכה משתמשת באותו Worker ובגבולות התעבורה הקיימים; HTTP רק רושם
בקשה ואינו קורא לספק. זו אינה הפעלת סריקת GuardDuty חדשה ואינה מגבלת Meta/AWS.
אין שינוי ב־10 ניסיונות ההורדה ל־60 שניות, ב־100 MiB או ב־120 שניות לנתיב הקובץ.
נדרשים Worker פעיל וחשבון מורשה כדי לבצע את הבירור בפועל.
[חוזה תפעול](s3-meta-media-quarantine.md), [חוזה API](vercel-railway-api-contract.md).

18.20 **הסרת עותק בהסגר, מדיניות Connect של Tal, 10.09.2026:** בקשה מפורשת
מותרת פעם אחת לכל משימת העלאה; Idempotency מונע יצירת עבודה נוספת. עד שלושה
ניסיונות לאותה גרסת S3, עם 60/120 שניות בין הניסיונות ו־Claim של עשר דקות.
עד חמש משימות בסדרה לסבב Worker, ודקת המתנה בין סבבים. עד פעולה אחת פעילה
במתאם האחסון; בכל ניסיון עד שש בדיקות Bucket ו־DELETE אחד. אין Retry פנימי
של SDK; ברירת המחדל היא Timeout של 30 שניות לכל בקשת S3. תלות שלא הסתיימה
אחרי Timeout ממשיכה לתפוס את המקום עד סיומה.

18.20.1 דחייה קבועה, לרבות 403/Object Lock, מסיימת ב־recovery-required; תוצאה
לא ידועה מקבלת רק את יתרת הניסיונות לאותה גרסה. תהליך cleanup משתמש בתפקיד
IAM נפרד. אין Polling או Retry אוטומטי בדפדפן ואין שינוי במכסות הורדה/בירור
או במגבלות Meta. אלו גבולות Connect, לא מגבלות ספק.
[חוזה האחסון וההפעלה](s3-meta-media-quarantine.md).
