...
| `data.relational-database` | D1/SQLite | PostgreSQL | Transaction executor, ‏Node driver, חוזה Pool מאובטח, ‏Foundation של 38 Adapters, ‏Readiness query ו־Runtime בעל סגירה מסודרת; Harness אמיתי עבר עם 23 Migrations, ‏Contact Consent evidence/projection, ‏Campaign Audience לקריאות `all/list/tag`, ‏Contact organization/import services, כתיבת Contact ותוצאת Import אטומית, ‏Conversation/Message inbox עם Idempotency ו־Optimistic locking, ‏Bot Flow, ‏Reply Delivery ו־Bot Runtime continuation/handoff, ‏Knowledge Source/Passage lifecycle אטומי, ‏AI Agent draft/version/source-link/publication, ‏AI Runtime cost/audit/handoff ו־Reply approval outbox אטומיים, ‏Tenant Subscription lifecycle עם Audit בלתי־משתנה, ‏Tenant/Owner/Profile/Audit provisioning אטומי עם חסימת התנגשות זהויות, ‏Production Decisions מוגבלות ל־Registry עם Event בלתי־משתנה, ‏System Admin directory/profile persistence, ‏Meta connection/Webhook receipts/Credential envelopes, ‏WhatsApp delivery-policy, ‏Reservation/Settlement/Cooldown evidence, ‏Worker scheduler lease, ‏Message Template lifecycle, ‏Campaign snapshot, ‏Dispatch ו־Provider reconciliation אטומיים, דוח HTTP מלא ו־57 תרחישי Concurrency. לא נותר פער Repository ממופה; נותרו ספק וערכי Pool חיים, ‏Parity מלאה מול 36 Migrations וראיות Staging |
