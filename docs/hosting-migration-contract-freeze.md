...
| `web.server-api-boundary` | Web ו־Business services באותו Worker | Vercel Web מול Railway API | Authenticated runtime, שלוש קריאות, Mutation מאובטח, PostgreSQL `contacts.list`, ‏Adapter חד־שאילתי ל־`reports.read`, ‏Foundation של 11 Adapters ו־Runtime composition הושלמו; מסלול HTTP מלא של הדוח נבדק מול PostgreSQL אמיתי. נותרו Node entrypoint, ‏Health/shutdown, ‏Live pool values, ‏Parity מלאה, יתר ה־Mutations, ‏Routes ו־Staging evidence |
...
| `data.relational-database` | D1/SQLite | PostgreSQL | Transaction executor, ‏Node driver, חוזה Pool מאובטח, ‏Foundation של 11 Adapters ו־Runtime בעל סגירה מסודרת; Harness אמיתי עבר עם תשע Migrations, דוח HTTP מלא, Contact/Conversation/Message/Template/Campaign/Bot/AI DML ושני תרחישי Concurrency. נותרו ספק וערכי Pool חיים, יתר ה־Repositories, ‏Parity מלאה מול 35 Migrations וראיות Staging |
