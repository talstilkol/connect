...
| `web.server-api-boundary` | Web ו־Business services באותו Worker | Vercel Web מול Railway API | Authenticated runtime, שלוש קריאות, Mutation מאובטח, PostgreSQL `contacts.list`, ‏Adapter חד־שאילתי ל־`reports.read` ו־Foundation של 11 Adapters הושלמו; Conversations, Messages, Templates, Campaigns ו־Bot deliveries הומרו ונבדקו, ונותרו שתי טבלאות AI לדוחות, ‏Live pool values, ‏Parity מלאה, יתר ה־Mutations, ‏Routes ו־Staging evidence |
...
| `data.relational-database` | D1/SQLite | PostgreSQL | Transaction executor, ‏Node driver, חוזה Pool מאובטח ו־Foundation של 11 Adapters; Harness אמיתי עבר עם Contact/Conversation/Message/Template/Campaign/Bot DML ושני תרחישי Concurrency. Adapter הדוחות קיים אך שתי טבלאות AI נותרו; נותרו ספק וערכי Pool חיים, יתר ה־Repositories, ‏Parity מלאה מול 35 Migrations וראיות Staging |
