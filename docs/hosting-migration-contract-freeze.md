...
| `web.server-api-boundary` | Web ו־Business services באותו Worker | Vercel Web מול Railway API | Authenticated runtime, שלוש קריאות, Mutation מאובטח ו־Critical-path PostgreSQL schema הושלמו; נותרו Driver, Parity מלאה, שאר ה־Mutations, Routes, Live configuration ו־Staging evidence |
...
| `data.relational-database` | D1/SQLite | PostgreSQL | Transaction executor ושתי Migrations מסודרות ל־Critical Path קיימים; נותרו ספק, Driver, Parity מלאה מול 35 Migrations וראיות מול Database אמיתי |
