...
| `web.server-api-boundary` | Web ו־Business services באותו Worker | Vercel Web מול Railway API | Authenticated runtime, שלוש קריאות, Mutation מאובטח, PostgreSQL `contacts.list`, ‏Adapter חד־שאילתי ל־`reports.read` ו־Foundation של 11 Adapters הושלמו; כל ששת מקורות הדוח הומרו ונבדקו מול PostgreSQL אמיתי. נותרו ‏Live pool values, ‏Parity מלאה, יתר ה־Mutations, ‏Routes ו־Staging evidence |
...
| `data.relational-database` | D1/SQLite | PostgreSQL | Transaction executor, ‏Node driver, חוזה Pool מאובטח ו־Foundation של 11 Adapters; Harness אמיתי עבר עם תשע Migrations, דוח מלא, Contact/Conversation/Message/Template/Campaign/Bot/AI DML ושני תרחישי Concurrency. נותרו ספק וערכי Pool חיים, יתר ה־Repositories, ‏Parity מלאה מול 35 Migrations וראיות Staging |
