...
| `web.server-api-boundary` | Web ו־Business services באותו Worker | Vercel Web מול Railway API | Authenticated runtime, שלוש קריאות, Mutation מאובטח, PostgreSQL `contacts.list`, ‏Contact Directory BFF מלא לקריאה עם Organization snapshot, ‏Contact profile save אטומי עם Idempotency ללא D1 fallback, ‏Contact Consent grant/unsubscribe BFF מלא על Evidence והקרנה אטומיים ללא D1 fallback, וארבע פעולות Contact organization עם Receipt, ‏Audit ו־Snapshot אטומיים ללא D1 fallback, ‏Campaign Audience לקריאות `all/list/tag`, ‏Contact import אטומי, ‏Conversation/Message inbox אטומי, ‏Bot Flow version/publication, ‏Reply Delivery ו־Bot Runtime continuation/handoff אטומיים, ‏Knowledge Source/Passage אטומיים, ‏AI Agent draft/version/source-link/publication, ‏AI Runtime cost/audit/handoff ו־Reply approval outbox אטומיים, ‏Tenant Subscription lifecycle עם Audit בלתי־משתנה, ‏Tenant/Owner/Profile/Audit provisioning אטומי עם חסימת התנגשות זהויות, ‏Production Decisions מוגבלות ל־Registry עם Event בלתי־משתנה ו־Railway query/mutation bridge, ‏System Admin directory/profile/subscription persistence וארבעה חיבורי Vercel BFF ללא D1 fallback, ‏Operational Reports BFF מלא ללא D1 fallback ועם מצבי Membership/Selection/Permission נפרדים, ‏Meta connection/webhook/credential ports, ‏WhatsApp delivery-policy evidence עם Kill switch ו־Reservation/Settlement/Cooldown ledger אטומי, ‏Message Template lifecycle, ‏Campaign snapshot, ‏Dispatch ו־Provider reconciliation אטומיים, ‏Adapter חד־שאילתי ל־`reports.read`, ‏Foundation של 40 Adapters, ‏Runtime composition וחוזי PostgreSQL token bucket נפרדים ל־Tenant, ‏System Admin ו־Meta webhook, ‏Node HTTP, ‏Health, ‏Shutdown owner, ‏`PORT`, ‏Signals ו־Startup executable הושלמו; Rehearsal אמיתי הוכיח Readiness ו־SIGTERM. מיפוי Source אוטומטי מכסה 36 מיגרציות ו־51 טבלאות D1. כל 10 ה־Data slices וכל 51 הטבלאות הוכחו מקומית, כולל Semantic parity, גבול Credential מוצפן, ‏Campaign snapshots קפואים, תוכן שיחות ובוט מוגן וקשרי AI/Governance/Audit מלאים. נותרו יתר Railway route wiring, ערכי Pool ו־Rate-limit policy חיים, ‏Export וחזרה בסביבה מבוקרת, יתר ה־Mutations ו־Staging evidence |
| `web.static-assets` | `ASSETS` | Vercel Web | תצורת Assets |
| `web.image-optimization` | `IMAGES` | Vercel Web | Adapter ובדיקות גבול |
| `api.meta-webhook-ingress` | Worker route | Railway API | Route Node חתום ומוגבל, PostgreSQL WABA lookup, מכסה משותפת ו־Queue port ספק־נייטרלי הושלמו מקומית; ללא Queue adapter, Secrets, ערכי Policy, Load ו־Staging route מחזיר 503 ואינו מוכן ל־Cutover |
| `data.relational-database` | D1/SQLite | PostgreSQL | Transaction executor, ‏Node driver, חוזה Pool מאובטח, ‏Foundation של 39 Adapters, ‏Readiness query ו־Runtime בעל סגירה מסודרת; Harness אמיתי עבר עם 26 Migrations ו־58 תרחישי Concurrency. Registry נבדק ממפה את כל 36 מיגרציות D1 ואת כל 51 טבלאות D1, ומסמן ארבע מיגרציות Railway-only. כל 10 ה־Data rehearsals ו־Semantic parity עברו עבור 51 טבלאות עם Counts/Digests. ‏Full-source preflight קורא את כל 51 הטבלאות מ־SQLite במצב Read-only ותחת Transaction יחיד. ‏Signed all-slice bundle מבצע את כל העשרה תחת Transaction יעד יחיד וכותב Receipt בלתי־משתנה. כלי Cutover דו־שלבי דורש אישור Source digest ו־Confirmation לפני טעינה; החזרה הריקה ודחיית Replay עברו מול PostgreSQL 16.13. לא נותר פער Repository ממופה; נותרו ספק וערכי Pool חיים, ‏Export חי, טעינה וחזרה בסביבה מבוקרת וראיות Staging |
