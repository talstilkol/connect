...
| `data.relational-database` | D1/SQLite | PostgreSQL | Transaction executor, ‏Node driver, חוזה Pool מאובטח, ‏Foundation של 39 Adapters, ‏Readiness query ו־Runtime בעל סגירה מסודרת; Harness אמיתי עבר עם 25 Migrations ו־58 תרחישי Concurrency. Registry נבדק ממפה את כל 36 מיגרציות D1 ואת כל 51 טבלאות D1, ומסמן שלוש מיגרציות Railway-only. כל 10 ה־Data rehearsals ו־Semantic parity עברו עבור 51 טבלאות עם Counts/Digests. ‏Full-source preflight קורא את כל 51 הטבלאות מ־SQLite במצב Read-only ותחת Transaction יחיד. לא נותר פער Repository ממופה; נותרו ספק וערכי Pool חיים, ‏Export חי, טעינה וחזרה בסביבה מבוקרת וראיות Staging |
