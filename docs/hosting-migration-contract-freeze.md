...
| `data.relational-database` | D1/SQLite | PostgreSQL | Transaction executor, ‏Node driver, חוזה Pool מאובטח, ‏Foundation של 39 Adapters, ‏Readiness query ו־Runtime בעל סגירה מסודרת; Harness אמיתי עבר עם 24 Migrations ו־58 תרחישי Concurrency. Registry נבדק ממפה את כל 36 מיגרציות D1 ואת כל 51 טבלאות D1, ומסמן שלוש מיגרציות Railway-only. תשעת ה־Data rehearsals הראשונים ו־Semantic parity עברו עבור 43 טבלאות עם Counts/Digests. לא נותר פער Repository ממופה; נותרו 8 טבלאות, ספק וערכי Pool חיים, ‏Semantic parity רחב וראיות Staging |
