...
אך הפעולות העסקיות ירוצו ב־Railway. כל בקשת JSON ביניהם חייבת להוכיח גם
...
3.5 עדכון 09.09.2026: רק בבקשת Mutation לפעולה
`meta.embedded-signup.complete` מותרים `wabaId` ו־`phoneNumberId`
כמחרוזות בשורש ה־Payload. אלה טענות נכסים מ־Embedded Signup,
המאומתות מול Meta לפני שמירה; הן אינן בוחרות Tenant. השדות נשארים
אסורים בפעולות אחרות, בקינון ובתגובות. `authorizationCode` מועבר
בבקשה זו בלבד במסלול המוצר החדש, ואינו נשמר ב־Receipt, ב־Audit
או בטלמטריית הבקשה. לאחר אימות הזהות, ההרשאה ומגבלת הפעולות,
נשמר Claim לפני בקשות הספק; processing אינו נשלח מחדש אוטומטית.
פרטי המימוש והגבולות מתועדים ב־[Master Plan, סעיף 20](planning/launch-master-plan-2026-09-09.md).

3.6 עדכון 09.09.2026: `meta.embedded-signup.begin` היא Mutation עם Payload ריק כברירת מחדל,
זהות שירות ומשתמש מאומתות, הרשאת workspace.manage ו־Rate Limit קיים.
היא מחזירה `{status:"ready", launchId, expiresAt}` או מצב חסימה מצומצם.
ההכנה משמרת ניסיון ready תואם ואינה פותחת Meta. התוקף המקומי הוא 20 דקות;
הוא אינו תוקף קוד Meta או חלון data sync. קוד הלקוח פותח FB.login בלחיצה נוספת.

3.7 `meta.embedded-signup.complete` דורשת כעת גם launchId מספרי חיובי.
ה־Tenant, המבצע, תצורת Meta וגרסת החיבור נבדקים בשרת. Claim של הקוד ושל
הניסיון נשמרים יחד, ורק קוד יחיד יכול לצרוך את הניסיון. Claim key נשאר מבוסס
על קוד ההרשאה ו־App ID; Request digest כולל גם launchId. לכן העברת אותו קוד
לניסיון חדש אינה מאפשרת שליחה חוזרת. לאחר תשובת Meta, שמירת נכסים היא CAS
מול גרסת החיבור שנשמרה בתחילת הניסיון. תוצאה ידועה סוגרת את הניסיון ואת
קבלת ההשלמה באותה טרנזקציה; תוצאה עמומה אינה מאפסת claimed.

3.8 נדרשת מיגרציה 0064 לפני ה־API החדש ופריסת Web תואמת. לקוח ישן שאינו
שולח launchId נדחה. Cloud API הוא מסלול ברירת המחדל; תמיכת Business App
בפיילוט מבוקר מתוארת בסעיפים 3.13–3.16. חידוש דור סנכרון עדיין חסום. אין שינוי בחתימות השירות,
ב־Contract version או במכסות Meta. פירוט בסעיף 29 של ה־Master Plan.

3.9 עדכון 09.09.2026: מיגרציה 0065 נדרשת לגרסת מאגר הסנכרון החדשה.
`prepareFromSignupLaunch` היא מתודה פנימית ב־Foundation, ללא HTTP operation.
היא גוזרת זמן, נכסים וגרסת חיבור מקבלת הרשמה שהצליחה, ושומרת קישור ושתי
בקשות סנכרון יחד. היא אינה שולחת POST ואינה מאפסת בקשות קיימות. היא נקראת
מתזמור Business App הפנימי. פרטי המימוש והאימות בסעיף 30 של ה־Master Plan.

3.10 עדכון 09.09.2026: נוסף Runtime פנימי ל־Business App שמחבר אירוע WABA
בלבד, קבלת הרשמה, הכנה ושתי בקשות סנכרון. resume משתמש בניסיון השמור
וב־Hash תצורה שכולל את המסלול, ללא קוד הרשאה חוזר. Runtime זה מחובר כעת
ל־HTTP רק כאשר הפיילוט מופעל בשרת; בברירת המחדל השלמת Business App מחזירה
synchronization-required. המשך ה־Worker והסכמה הנדרשת מפורטים להלן.

3.11 עדכון 09.09.2026: מיגרציה **0066** מוסיפה משימות המשך ל־Coexistence.
השלמת Business App הפנימית שומרת את המשימה ואת קבלת ההצלחה באותה טרנזקציה.
תחזוקת ה־Worker של PostgreSQL, עם metaWebhooks מוגדר, ממשיכה עד 10 משימות
בכל הרצה. נדרשות הגדרות Meta זהות ל־API, כולל
`META_EMBEDDED_SIGNUP_CONFIGURATION_ID` ומפתח ה־Credential Vault; ללא תצורה
תקינה אין בקשת ספק, והמשימה ממתינה לבדיקה חוזרת בתוך החלון המקורי.
אין שמירת קוד הרשאה או Token במשימה ואין צורך בדפדפן פתוח.

3.12 ה־Worker טוען מחדש את חברוּת המבצע המקורי ואת הרשאותיו. Claim של POST
בודק שוב בעלים פעיל ואת מצב המשימה תחת נעילת מסד, אחרי GET מול Meta.
ביטול משימה סופי אינו מתבטל בעקבות החזרת הרשאה. תוצאת POST שנשלח כבר
נשמרת גם כאשר ההרשאה משתנה בזמן הבקשה. החזרת המשימה לתור אינה מאפסת
בקשות accepted/dispatching/unknown; לא מתבצע POST חוזר בשל חוסר ודאות.
יש להחיל 0066 לפני ה־API/Worker החדשים. אין Backfill של משימות להרשמות
היסטוריות, אין חידוש דור סנכרון ואין שינוי ב־Contract version. הפיילוט החדש
אינו הוכחת QR חי או אישור השקה ציבורית. פירוט וראיות בסעיפים 32–33 של ה־Master Plan.

3.13 הפעלה מבוקרת: `META_COEXISTENCE_ONBOARDING_MODE=controlled-pilot`
בשרת ה־API, יחד עם תצורת Meta תקינה, מאפשרת את מסלול Business App.
ערך חסר, ריק או אחר משאיר אותו חסום. תשובת configuration מוסיפה אז בלבד
`businessAppEnabled: true`; שדה חסר משאיר את הבחירה בממשק מושבתת.
זהו מתג ברמת סביבת ה־API, ללא Allowlist נפרדת של Tenants. הוא חל על המשתמשים
בעלי ההרשאה המתאימה באותה סביבה ואינו מעיד על זכאות החשבון אצל Meta.
הסבב הזה הוסיף את ההגדרה ל־`.env.example` בלבד; לא הופעל מתג בסביבה חיה.
כיבוי המתג חוסם הרשמות Business App חדשות ואינו מבטל משימות המשך שנשמרו.

3.14 `begin` מקבל `{}` עבור Cloud API, או בדיוק `{flow:"cloud-api"}` /
`{flow:"business-app"}`. שדות נוספים וערכי flow אחרים נדחים. ההרשאה ומגבלת
הפעולות נבדקות לפני תחילת ניסיון. השלמת Business App מקבלת קוד הרשאה,
launchId, flow ו־wabaId בלבד; בעלים ומספר טלפון שמגיעים מהלקוח נדחים.
השרת פותר ומאמת את הנכסים מול Meta. המסלול קשור ל־Hash הניסיון, והוספת
businessAppEnabled לתצוגה אינה משנה את Hash התצורה או את זהות ניסיונות Cloud קיימים.

3.15 הצלחת Business App מחזירה
`{status:"connected", connection:{status:"connected"}, synchronization:"background"}`
לאחר Commit של קבלת ההרשמה ומשימת ההמשך. ה־HTTP אינו ממתין ל־POST של אנשי
הקשר וההיסטוריה; ה־Worker ממשיך אותם מהמסד. background הוא אישור שמירת
ההמשך, לא מצב התקדמות נוכחי ולא הוכחת ייבוא. גם Replay של קבלת הצלחה מחזיר
אישור זה בלי להחליף שוב קוד. הלקוח דורש אותו בהצלחת Business App ודוחה אותו
בהצלחת Cloud API. לא נוספה פעולת HTTP או גרסת חוזה חדשה.

3.16 קריאת מצב הסנכרון תומכת ב־`awaiting-worker` לפני יצירת בקשות הספק:
אנשי קשר והיסטוריה במצב not-prepared, ללא אחוזים ועם מונים אפסיים. הקריאה
מאמתת משימה, קבלת הצלחה, Tenant וגרסת חיבור נוכחית; ביטול או שינוי חיבור
מוצגים כמצבי התאוששות מתאימים. חלון הממשק מרענן קריאת מצב עד 12 פעמים,
במרווחי 5 שניות כשהוא פתוח והדף גלוי; אחר כך נותר רענון ידני. יש לפרוס API
תואם וסכמה עד 0066 לפני הפעלת הפיילוט ב־Web, ולהפעיל Worker עם תצורה תואמת.
יש לשדרג את צד ה־Web הקורא את מצב הסנכרון לפני יצירת עבודות הפיילוט.
API ישן שאינו מפרסם businessAppEnabled משאיר את האפשרות מושבתת בממשק החדש.
האימות המקומי והגבולות מתועדים בסעיף 33 של [Master Plan](planning/launch-master-plan-2026-09-09.md).

3.17 מיגרציה **0067** נדרשת לפני ה־Worker הכולל שיוך מדיה היסטורית.
`meta_history_media_bindings` שומרת הפניות בלתי ניתנות לשינוי להודעה המקורית
ולאירוע המדיה. תחזוקת ה־Worker משייכת עד 100 זוגות להרצה, לאחר הקרנת ההיסטוריה.
`readBoundMedia` היא קריאה פנימית של Repository: היא מאמתת מחדש את החיבור,
בקשת ההיסטוריה, מצב השיתוף ומקורות התוכן, ואינה HTTP operation או הרשאה
לשימוש מאוחר ב־Token. קישור שמור אינו מאפשר גישה לאחר סירוב, ניתוק, סתירה,
מחיקה יוצאת או הופעת עותק חי. אין העתק נוסף של תוכן בקישור ואין שדות מדיה
חדשים בתשובת API. הממשק ממשיך להציג media_placeholder עד למימוש זמינות הקובץ;
לא נוספו הורדה, אחסון קבצים, Preview או סימון סנכרון complete. פירוט בסעיף 34
של [Master Plan](planning/launch-master-plan-2026-09-09.md).

3.18 הורדת מדיה פנימית נוספה ב־`createRailwayMetaHistoryMediaRuntime`.
ה־Runtime מאמת חברות ומקור אחרי פענוח Credential ולפני כל GET, מבצע בדיקת
בעלות באמצעות phone_number_id, ובודק שוב הרשאה לאחר קבלת Bytes תקינים.
תוצאה פנימית כוללת quarantineRequired:true בלבד; אין HTTP operation חדש,
שדות DTO חדשים, אחסון פרטי, סריקה או הפעלת הורדה ב־Worker. הביטוי
"לא נוספה הורדה" בסעיף 3.17 מתאר את מצב המימוש לפני תוספת זו.
לפני שמירה והגשה עתידיות נדרש אימות הרשאה נוסף; התוצאה בזיכרון אינה היתר
מתמשך. הממשק ממשיך להציג Placeholder ואין טענה לייבוא complete. לא נוספה
מיגרציה; הסכמה נשארת 68 מיגרציות עד 0067. מימוש, מגבלות ובדיקות בסעיף 35
של [Master Plan](planning/launch-master-plan-2026-09-09.md).

3.19 נוסף `createRailwayMetaHistoryMediaQuarantineRuntime` פנימי ל־S3.
הוא מחבר הורדה מאומתת לשמירה מותנית עם KMS ו־VersionId, ובודק הרשאה מחדש
גם אחרי המתנת AWS Credentials ולפני Transport. התוצאה היא Receipt פנימי
במצב quarantined, ללא Bytes או כתובת הגשה. אין HTTP operation חדש, Worker
מוריד, טבלת קבלות עמידה או שינוי DTO. הסכמה נשארת 68 מיגרציות עד 0067.
יש להשלים יומן כוונות ו־Claims, בירור כתיבות, סריקה והרשאת הגשה לפני הפעלה.
תצורה ומדיניות חובה מפורטות ב־[חוזה S3](s3-meta-media-quarantine.md).

3.20 מיגרציה **0068** מוסיפה יומן העלאות עמיד, Claims ואישורים לפי גרסת
אובייקט. `createRailwayMetaHistoryMediaQuarantineRuntime` מחייב כעת
`uploadJournal`; יש להחיל את הסכמה ולספק Repository PostgreSQL לפני הפעלתו.
אישור תקין נשמר לפני בדיקת ההרשאה שלאחר PUT, וחזרה לאישור שמור אינה פונה
ל־Meta או ל־S3. שיגור ללא אישור אינו מתאפס אוטומטית. הסכמה כוללת כעת
**69 מיגרציות**, מהן **40 ליעד בלבד**. סעיף זה מעדכן את היעדר טבלת הקבלות
בסעיף 3.19; אין שינוי HTTP operation או DTO, ואין עדיין הפעלת הורדת מדיה
ב־Worker, קליטת סריקה או הגשת קובץ. יש להשלים בירור כתיבה ללא אישור,
סריקה והרשאת הגשה לפני הפעלה. פירוט בסעיף 37 של
[Master Plan](planning/launch-master-plan-2026-09-09.md).

3.21 מיגרציה **0069** מוסיפה תצפיות סריקה בלתי ניתנות לשינוי לפי משימה
וגרסת אובייקט. Runtime פנימי חדש `createRailwayMetaMediaInspectionRuntime`
מחבר PostgreSQL לבירור S3 ולקריאת תגית GuardDuty. אישור חסר משוחזר רק
לאחר אימות גרסה, תגית, checksum ומטא־נתונים; התצפית והאישור נשמרים אטומית.
ה־Runtime מחזיר pending/clean-observed/blocked/conflict/missing פנימיים,
ללא שדות HTTP או DTO חדשים וללא הרשאת הגשת קובץ. הסכמה כוללת כעת
**70 מיגרציות**, מהן **41 ליעד בלבד**. יש להחיל 0069 לפני הפעלת השירות.
אין הפעלה אוטומטית ב־Worker או קליטת אירועים מהדפדפן. ראו סעיף 38 של
[Master Plan](planning/launch-master-plan-2026-09-09.md).


3.22 מיגרציה **0070** מוסיפה תור מדיה עמיד, עם Claim, מקור ומבצע קבועים,
השהיה ומיצוי ניסיונות. META_MEDIA_WORKER_MODE בוחר upload או inspect
במפורש; היעדרו משאיר את המסלול כבוי. לולאת מדיה עצמאית מחוברת למחזור
...
3.23 הפעולה **meta.media-tasks.read** היא query עם idempotencyKey=null
וקלט ריק או after={jobKey,kind}. Tenant אינו מתקבל בקלט; הוא נפתר מהמשתמש
והארגון המאומתים. נדרשת workspace.manage ובדיקת בעלים פעיל ומצב Tenant
באותה שאילתת PostgreSQL שקוראת את הרשומות. Cursor חייב להשתייך לאותו
Tenant. התשובה היא {page:{tasks,nextCursor}} עם עד 50 משימות, מצב,
ניסיונות, זמנים, Lease שפג וראיות סריקה היסטוריות. כל שדה נוסף נדחה.
אין מזהי Meta/S3, מבצע, Tenant, תוכן, Token או URL בתשובה; אין כתיבה או
בקשת ספק. מסך /workspace/media-tasks משתמש ב־BFF שרתי וברענון ידני.
הקריאה מותרת לבעלים גם אחרי ניתוק Meta ואינה היתר לקריאת קובץ.
[חוזה DTO](../shared/domain/metaMediaTaskView.ts), [Master Plan, סעיף 40](planning/launch-master-plan-2026-09-09.md).

3.24 קריאת מדיה מאומתת נוספה כשירות פנימי עם withFile. היא דורשת
conversations.read לפי Membership ומקור עדכניים, גרסה שמורה וסריקה נוכחית.
אין Operation JSON חדש, תגובת Base64, URL אחסון או שדה זמינות ב־Inbox.
גבול התשובה המקומי ב־API נשאר 1 MiB. נתיב בינארי נפרד ב־Railway מומש בסעיף 3.25 עם אימות זהות ו־Tenant,
ביטול העברה, Backpressure וכפתור ב־Inbox. הפעלה חיה דורשת תצורה וראיות ספק. צרכן פנימי חייב לסיים שימוש ב־Bytes
לפני השלמת withFile, משום שה־Buffer מאופס בסיום.
[Runtime](../server/platform/railwayMetaMediaFileReadRuntime.ts), [Master Plan, סעיף 41](planning/launch-master-plan-2026-09-09.md).

3.25 **הורדת מדיה בינארית — 10.09.2026:** GET מדויק ל־`/v1/media-files/<messageKey>`
הוא נתיב נפרד מה־JSON API. הדפדפן פונה ישירות ל־Railway עם Clerk session
ב־Authorization, בלי Cookie, Query credential, Tenant, יעד S3 או Vercel OIDC.
ה־JSON API ממשיך לדרוש את שתי ההוכחות. CORS מאפשר רק APP_PUBLIC_ORIGIN,
GET ו־Authorization; Origin חסר/זר, Range, Credentials כפולים ונתיב לא קנוני נדחים.
Clerk מאמת session_token ו־authorizedParties; Membership, בחירת סביבת העבודה
וקישור ארגון Clerk נפתרים בשרת ונבדקים שוב לפני בקשות S3 ומסירת התוכן.

3.25.1 META_MEDIA_FILE_READ_MODE=enabled מפעיל את הנתיב רק עם תצורת S3 תקינה;
היעדר הערך או disabled משאירים אותו כבוי. NEXT_PUBLIC_META_MEDIA_DOWNLOAD_ORIGIN
מוגדר בזמן Build של ה־Web כ־Origin HTTPS קנוני של Railway. ב־Development בלבד
מותר HTTP ב־loopback. הכפתור מופיע רק תחת אימות מופעל, ארגון נוכחי והודעת מדיה
היסטורית שלא נערכה/נמחקה. הכפתור אינו הבטחת זמינות; השרת מכריע בכל ניסיון.

3.25.2 לפני S3 נשמר Audit מסוג meta.media.file.read-admitted תחת נעילות מקור
והרשאה עדכנית. עד 10 ניסיונות מורשים בדקה למשתמש ול־Tenant, לפי שעון PostgreSQL.
הספירה משותפת למופעי API. רישום זה מוכיח קבלת ניסיון, כולל ניסיון שנכשל או בוטל;
אינו מוכיח שהקובץ הורד או נשמר. אין רישום של Token או מזהי אחסון.

3.25.3 עד 100 MiB, בקשה פעילה אחת למופע ו־120 שניות לבקשה הכוללת. Node מעתיק
מקטע של עד 64 KiB בכל פעם וממתין לסיום כתיבתו ול־finish; אחרי סיום הצרכן ה־Buffer
מאופס. ניתוק דפדפן, ביטול ו־Timeout עוצרים את הזרם. הגבלת זמן משיבה כשל גם אם
תלות מאחרת, אך אינה משחררת את מכסת הפעולה הפעילה לפני שהתלות מסתיימת. הרשאה
נבדקת לפני תחילת המסירה; אין החזרה או ביטול בדיעבד של Bytes שכבר נמסרו.

3.25.4 התגובה היא application/octet-stream, attachment בשם media.bin ו־private,
no-store. שגיאות הן טקסט קבוע ללא פרטי ספק: 401 בזהות חסרה, 403 בהרשאה שנדחתה,
409 בקובץ שאינו זמין, 429 במיצוי מכסה, 503 בעומס/תלות/ביטול. כשל פתירת Session
עשוי להחזיר 503 בלי לחשוף אם ארגון קיים. 429 כולל Retry-After:60, ו־503 כולל 5.
אין Preview פעיל, Range, Base64 או קישור S3 חתום. נדרשים מבחני פריסה לקבצים גדולים,
הגבלת תעבורה חיה, Clerk/AWS/GuardDuty ו־CORS בין הדומיינים האמיתיים.
[Master Plan, סעיף 42](planning/launch-master-plan-2026-09-09.md).

3.26 **בקשת בדיקת סריקה נוספת — 10.09.2026:** הפעולה meta.media-inspection.retry היא
mutation דרך אותו POST JSON של Vercel → Railway, עם Clerk Bearer ו־Vercel OIDC.
הקלט הסגור הוא {jobKey,expectedVersion}; Tenant, מבצע, יעד S3 ומספר הניסיונות
אינם מתקבלים מהדפדפן. Idempotency הוא connect_idempotency_v1_ עם SHA-256 של
הפעולה והקלט הקנוני. התשובה הסגורה היא {status:"queued"} או {status:"already-requested"}.
ה־BFF ממפה כשלי הרשאה, גרסה, קלט ותלות להודעות קבועות בשלוש שפות.

3.26.1 נדרשים workspace.manage, בעלים פעיל כעת, מבצע ההעלאה המקורי שעדיין בעלים,
Tenant ומקור תקפים. משימת inspect חייבת להיות recovery-required לאחר מיצוי
12 הניסיונות האוטומטיים ועוד הבקשות שכבר ניתנו. כל בקשה מפורשת מעניקה ניסיון אחד,
עד שלוש בקשות למשימה. גרסה מיושנת, כשל קבוע מוקדם, משימת upload, תוצאה חוסמת
בהיסטוריה או ריבוי גרסאות סריקה אינם זכאים. העובד בודק שוב מקור, מבצע ו־Claim.

3.26.2 מיגרציה 0071 שומרת בקשה בלתי ניתנת לשינוי, מעבר לתור ו־Audit באותה
טרנזקציה, עם אילוץ שנבדק ב־Commit. בקשה זהה שנקלטה מוחזרת already-requested גם
אם העובד כבר התקדם; הרשאת המקור והבעלים עדיין נדרשת. ה־HTTP אינו מפעיל S3/Meta.
המונים מצטברים; המבצע, ההעלאה וראיות הסריקה נשמרים. אין התחלת סריקת GuardDuty חדשה.

3.26.3 DTO האבחון מוסיף version ו־operatorRetries. נדרשת פריסה מתואמת של 0071,
API/Worker ו־Web: הצרכנים משתמשים בסכמה סגורה ואין תאימות שקטה בין DTO ישן לחדש.
יש להשהות Worker בתהליך העדכון, להחיל את המיגרציה, לפרוס את השרתים ואת ה־Web,
ואז להחזיר את inspect לאחר בדיקות הבריאות. כש־META_MEDIA_WORKER_MODE כבוי,
בקשה נשמרת בתור בלבד. אין מתג הפעלה חדש ואין שינוי במדיניות הבינארית של 3.25.
[מימוש ותוכנית, סעיף 43](planning/launch-master-plan-2026-09-09.md),
[הפעלה ומגבלות](s3-meta-media-quarantine.md).

3.27 **הסרת עותק מדיה ביוזמת בעלים — 10.09.2026:** הפעולה meta.media-cleanup.request
היא mutation דרך BFF עם Clerk ו־Vercel OIDC. הקלט הסגור כולל jobKey,
expectedVersion ו־confirm="remove-quarantined-copy" בלבד; הלקוח אינו בוחר Tenant,
Bucket, Key או VersionId. מפתח SHA-256 נגזר מהפעולה ומהקלט הקנוני. HTTP שומר
כוונה ו־Audit באותה טרנזקציה, ללא פנייה לספק. הצלחה מאשרת רישום בקשה בלבד.

3.27.1 נדרש בעלים מבקש פעיל. משימת inspect צריכה להיות blocked או recovery-required,
בלי upload/inspect פעילים, עם אישור העלאה לגרסה אחת וללא ראיה לגרסה אחרת.
הסרה מותרת גם אחרי ניתוק המקור או ביטול בעלות המבצע המקורי; זהותו ביומן נשמרת.
הגרסה והניסיונות אינם מתאפסים. החל מ־Commit נחסמות קריאה, העלאה חוזרת ובירור.
ביטול ההסרה או תוצאה לא ידועה אינם משחזרים גישה. בקשה כפולה זהה מוחזרת
already-requested; גרסה ישנה או בקשה מתחרה מוחזרות conflict.

3.27.2 DTO האבחון מוסיף canCleanup ו־cleanup, שהוא null או אובייקט עם status
ו־attempts. הסכמה, Web ו־API/Worker חייבים להתעדכן יחד עם 0072. מצב cleanup
מופעל בתהליך נפרד עם תפקיד IAM מתאים; ברירת המחדל נשארת כבויה. Worker מאמת
את בעלות המבקש שוב לפני DELETE לגרסה שנשמרה בלבד. הרשאה שבוטלה בזמן תעבורה
אינה מוחקת אישור ספק שכבר התקבל; סיום עדיין דורש Claim תקף ו־Audit.

3.27.3 removed פירושו ש־S3 אישר הסרה של הגרסה המבוקשת, לרבות גרסה שכבר אינה
קיימת. אין הוכחת מחיקה מכל גיבוי או מטלפון, אין עקיפת Object Lock ואין ניקוי
אוטומטי לפי גיל. מקרים עם גרסה חסרה/סותרת נשארים לבירור תפעולי.
[חוזה האחסון](s3-meta-media-quarantine.md),
[מימוש ואימות, סעיף 44](planning/launch-master-plan-2026-09-09.md).
