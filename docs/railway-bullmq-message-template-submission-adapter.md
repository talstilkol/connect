...
8.5 סנכרון Meta ידני מחובר ל־Railway לפי סעיף 9. ההפעלה שלו נפרדת
מהגשה ונשארת כבויה כברירת מחדל. אין להסיק מהפעלת הגשה שסנכרון הופעל.

## 9. סנכרון ידני אטומי — 10.09.2026

9.1 MESSAGE_TEMPLATE_SYNC_ENABLED=true מדויק דורש META_GRAPH_API_VERSION
ומפתח META_CREDENTIAL_ENCRYPTION_KEY_V1 תקינים. תצורה פגומה מפילה את
ה־Executable לפני חיבור לספקי התשתית. canSync ב־templates.list דורש גם
templates.write; templates.sync אוכף את ההרשאה והתצורה גם בקריאה ישירה.
יש לפרוס API לפני Web. API ישן ללא canSync משאיר את כפתור הסנכרון כבוי.

9.2 GET מתבצע לפני פתיחת הטרנזקציה, דרך הכספת וה־Adapter הקיימים עם
מגבלות העימוד שלהם. בזמן הכתיבה נבדקים וננעלים שוב העסק, החברות,
החיבור ו־Credential envelope. שינוי בזמן GET דוחה את הכתיבה. תוצאות
הסטטוסים, Audit ו־Receipt נשמרים בטרנזקציה אחת; כשל מבטל את כולם.
תוצאת Meta אינה כוללת כאן יצירת טיוטות חדשות עבור תבניות לא תואמות:
ה־Sync משמר את סמנטיקת unmatched הקיימת.

9.3 requestedAt נוצר בדפדפן ומשמש לזהות ניסיון בלבד. זמן התצפית נקבע
בשרת לפני GET, כדי שתשובה איטית לא תדרוס אירוע חדש יותר. לאחר כשל רשת
המסך שומר את requestedAt לניסיון חוזר. Receipt שהושלם מחזיר את אותה
תוצאה ללא GET נוסף, לאחר בדיקת הרשאות עדכנית. זהו Snapshot של הניסיון
המקורי; סנכרון חדש לאחר הצלחה מקבל זמן בקשה חדש. אין שימוש באקראיות.

9.4 אין לשנות את המתג בספק לפני זמינות API, מפתח פענוח, הרשאות Meta
וחיבור מאומת ב־Staging. המתג אינו בדיקת בריאות של Meta. ה־Worker נדרש
להגשה, לא ל־GET של סנכרון ידני. אין כאן הפעלה חיה או אישור תבנית אמיתי.
השבתת המתג מונעת בקשות חדשות ואינה מבטלת בקשה שכבר נכנסה לביצוע.

9.5 הבדיקה tests/integration/message-template-sync-postgres.test.mjs דורשת
מסד מקומי ריק בשם connect_startup_rehearsal דרך
CONNECT_POSTGRES_STARTUP_REHEARSAL_URL. היא מחילה את כל המיגרציות,
ובודקת כפילות מקבילה, SQL rollback, אירוע חדש יותר, ביטול הרשאה ושינוי
חיבור. CI מריץ אותה מול שירות PostgreSQL זמני נפרד מזה של בדיקת Startup.
המסד אמיתי; תגובות הספק משתמשות בנתוני הבדיקות הקיימים ואינן Meta חי.
