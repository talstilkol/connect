...
1.6.2 **סדר ההמשך:** אימות חשבונות → זיהוי משאבים ובידוד Staging → מסד והרשאות/TLS → תצורת זהות ו־Meta → פריסת API/Web/Worker ובריאות → קבלה חיה של QR/קוד ותהליכי הליבה. משימות 2.2–4.9 ותנאי הסיום שלהן נשארים מחייבים; חבילה מקומית אינה סוגרת שלב של שילוב חי.
