...
## 1.6 מצב נוכחי — 11.09.2026

**עדכון R221:** ניקוי מדיה לפי מדיניות, גיל/סגירה וגרסאות לא ידועות מומש מקומית, כולל ביצוע אוטומטי מוגבל והרשאות פרטיות. נותרו חיבור מחדש עם שיוך מחזור סנכרון ובדיקת מסירה סופית — שני תתי־שלבים, 8–16 שעות הנדסה בביטחון נמוך. 12 שלבי ההשקה עדיין פתוחים. [התוכנית הפעילה](code-completion-plan-2026-09-11.md#16-r221--ניקוי-מדיה-והכרעת-מדיניות-הייבוא-12092026), [תפעול](../meta-media-retention.md).

**עדכון סדר עבודה מחייב — תכנות לפני חיבורים:** לפי הוראת Tal האחרונה, להשלים תחילה את עבודת הפיתוח המקומית האפשרית גם בשלבים 10–11. חיבור החשבונות, פריסה וקבלה חיה נדחים לסוף הפיתוח. זהו שינוי בסדר הביצוע, לא ויתור על Pilot, QR/קוד אמיתי או בדיקות ספק. [תור הקוד העדכני והאומדנים](code-completion-plan-2026-09-11.md) מפרט את סעיף 82 של תוכנית זו.

**גישות שכבר אומתו:** GitHub ו־Vercel; Railway אומת בהצלחה ב־11.09. נמצאו שני פרויקטים אחרים וללא Connect. בצוות Vercel raceisrael נמצאו 0 דומיינים. טרם נפרס Connect. תיאורי חסמי Railway בהמשך המסמך הם היסטוריים. [ראיית מצב החשבונות](../../outputs/launch-validation-2026-09-09/provider-connection-inventory-20260911.json).

1.6.1 **חסם Vercel נפתר ב־11.09.2026:** ההתחברות החמישית הושלמה, ו־whoami וקריאת הצוותים/הפרויקטים אימתו את החשבון. נוצר פרויקט ייעודי `connect-staging` בצוות `raceisrael`; Next.js, Node 24 ופקודות הבנייה מהמאגר הוחלו ואומתו בקריאה חוזרת. [ראיית החשבון](../../outputs/launch-validation-2026-09-09/staging-access-20260911-vercel-authenticated-discovery.json), [ראיית ההגדרות](../../outputs/launch-validation-2026-09-09/staging-access-20260911-vercel-project-settings.json). טרם הועלה קוד או בוצע Deployment. גישה ל־Railway כבר אומתה; השלמת תצורת Staging וזהות/Meta תתבצע לאחר תור הפיתוח המקומי. ארבעת ניסיונות Vercel הקודמים נשארים היסטוריים; אין צורך בקוד Vercel חדש.

| רכיב | מה הוכח | מה עדיין חסר |
|---|---|---|
| גרסת המוצר | main ‏671f8183; עשרת Checks עברו, 4,505 בדיקות כלליות, שני Builds ו־401 בדיקות PostgreSQL/Redis. [ראיות הגרסה](../../outputs/launch-validation-2026-09-09/echo-original-caption-validation.json) | פריסת אותה גרסה ואימות זהותה בשירותים; בדיקת תוקף ראיות השחרור בזמן הפריסה |
| אתחול מסד | חבילה עם 76 מיגרציות; תמונת Linux AMD64 מקומית; 16 בדיקות מבצע ו־13 בדיקות SQL עברו. [חבילת ההפעלה](../../outputs/launch-validation-2026-09-09/postgres-bootstrap-runner-671f8183/README.md) | מסד Staging אמיתי, תפקידים, קובצי סיסמה ו־CA בזמן Runtime, TLS מאומת והרצה אצל הספק |
| סביבות | גישת GitHub ו־Vercel הוכחה; פרויקט connect-staging קיים ב־Vercel עם הגדרות Build מאומתות | גישה ו־Project/Environment ב־Railway; Clerk ו־Meta אמיתיים; משתני סביבה ופריסת השירותים. גישת Vercel אינה גישה לספקים האחרים |
| חיבור WhatsApp | מסלול Coexistence וממשק החיבור קיימים בקוד, עם בדיקות הרשמה וסנכרון | QR/קוד, זכאות מכשיר, קליטה ושליחה מורשות מקצה לקצה ב־Staging |

1.6.2 **סדר חיבורי הספקים לאחר השלמת הפיתוח המקומי:** אימות חשבונות → זיהוי משאבים ובידוד Staging → מסד והרשאות/TLS → תצורת זהות ו־Meta → פריסת API/Web/Worker ובריאות → קבלה חיה של QR/קוד ותהליכי הליבה. משימות 2.2–4.9 ותנאי הסיום שלהן נשארים מחייבים; חבילה מקומית אינה סוגרת שלב של שילוב חי.

1.6.3 **זמן שנותר:** 12 שלבים ראשיים, עם תתי־משימות שמומשו ועדיין טעונות קבלה. הבדיקה הראשונית של חשבון Vercel הושלמה. יתרת הפיתוח וזמן ההשקה אינם ידועים לפני בדיקת הסביבות. טווחי השעות והשבועות בסעיפים 4–6 הם בסיס התכנון מ־09.09, ולא אומדן עדכני של היתרה. הפיילוט בן 10 ימי העבודה ותצפית ה־Canary בת 72 השעות עדיין חלק מהתוכנית.

1.6.4 **המשך קוד מקומי, 11.09:** לאחר הקמת פרויקט Vercel קודם תיקון קריאת קובץ קיים אחרי עריכת כיתוב, לפי R206 וסעיף 80. [מצב האימות והפרסום](../../outputs/launch-validation-2026-09-09/media-caption-file-validation.json) נפרד מראיות main ‏671f8183. קוד Railway הראשון פקע ללא אימות; אין Deployment או קבלת QR.

1.6.5 **עדכון 11.09:** תיקון R206 מוזג ל־main ‏03c0104b; עשרת Checks עברו גם על Commit המיזוג, עם 4,506 בדיקות כלליות ו־408 PostgreSQL/Redis. [ראיות המיזוג](../../outputs/launch-validation-2026-09-09/media-caption-file-validation.json). בזמן המתנה להתחברות Railway מקודם סיכום Dashboard לפי R207 וסעיף 81. אין Deployment, אימות Railway או קבלת QR בסבב זה.

# 2. תמונת הבסיס ההיסטורית — 09.09.2026
...
Handoff. ארבעת פערי G01–G04 כוללים כעת מימוש בקוד: הגשת/סנכרון Templates,
מוכנות Campaign delivery, Pause/Resume/Cancel ומענה ידני. כולם עדיין דורשים
קבלה חיה. תיקון שילוב של שמירת בקשות ותיאום רענונים מפורט בסעיף 65;
המצב העדכני מופיע גם ב־[מפת התהליכים](workflow-implementation-map-2026-09-10.md).
...
# 64. מימוש G04 — תשובות ידניות, 10.09.2026

64.1 מומש מסלול טקסט ידני מה־Inbox דרך Server Action, API עם הרשאה ו־Quota, Outbox ייעודי ו־Worker. מיגרציה 0073 מעלה את מלאי PostgreSQL ל־74 מיגרציות; 45 מהן ליעד בלבד. בקשת השמירה, Receipt, Audit וגרסת השיחה אטומיים. ה־API כולל כעת 35 פעולות ברישום המרכזי.

64.2 נדרש שיוך השיחה לנציג, חברות ותפקיד עדכניים, חלון שירות ו־Meta binding/credential תקפים. לפני ספק נבדקים שוב גם Contact version ו־Credential revision/digest. אין מזהה ספק או הודעת Inbox פיקטיבית לפני קבלת Provider Message ID. Reservation משתמש במגבלות Service Reply הקיימות.

64.3 החלטה על POST יחיד: Claim שפג לפני שליחה ניתן לתביעה מחדש; Seal שהושלם אינו נשלח שוב. אובדן תגובה או שמירת קבלה נשאר unknown וחוסם תשובה נוספת עד בדיקה. אין במוצר כפתור לשחרור unknown ללא הוכחת ספק. תרגול התאוששות מול ספק נשאר תנאי קבלה פתוח.

64.4 הבדיקות כוללות שמירה מקבילית, Rollback, Replay, הרשאות, חלון שירות, דור Claim, קריסה אחרי Seal, אמת ספק מאוחרת ועדכוני מסירה. הבדיקה המלאה הראשונה העבירה שתי בניות ו־4,469 מתוך 4,473 בדיקות. ארבעת הכשלים היו מלאי סכמות/Repositories שלא עודכן למיגרציה ול־Adapter החדשים; הציפיות ומרשם המקורות עודכנו. לא שונו כללי הייצור כדי לבטל בדיקת כשל.

64.5 בבדיקות PostgreSQL הראשונות נמצא setup ישן עם מזהי Meta לא קנוניים, ללא Tenant barrier, או ללא עדכון גרסת חברות/שדות הסכמה. ה־Fixtures הותאמו לחוזים הקיימים, בלי החלשת Constraint או Trigger. 12 בדיקות PostgreSQL עברו בהמשך, כולל מרוץ אמיתי מול ביטול הרשאה. תוצאות מוקדמות נשמרו בנפרד.

64.6 [חוזה מימוש, פריסה ותפעול](../railway-manual-replies.md) מפרט Worker → API → Web אחרי מיגרציה, מתג כבוי כברירת מחדל, תנאי הפעלה מבוקרת וגבולות unknown. לא בוצעה שליחה חיה, חיבור QR, פריסה או אימות UI בדפדפן. לא נעשה ניסיון נוסף לגשת למחשב שננעל.

64.7 ההכרעות R165–R168. ארבעת פערי הקוד G01–G04 כוללים כעת מימוש מקומי, אך בדיקות ספק, דפדפן, התאוששות ותיקוני שילוב עדיין פתוחות. אין לסמן את שלב 5 כולו או את הפיילוט כ־Accepted. נותרו 12 שלבי־על. אומדן 16–32 שעות של G04 הוא בסיס התכנון המקורי, לא יתרה חדשה; זמן הסיום הכולל אינו ידוע. הפעולה הבאה היא השלמת תנאי הקבלה והמוכנות בסעיפים 3–5, עם המשך בדיקות מקומיות שאינן תלויות בחשבונות.

64.8 [דוח אימות הגרסה](../../outputs/launch-validation-2026-09-09/manual-reply-validation.json) ו־[מסמך ההכרעות והתוצאות בצד](../../outputs/launch-validation-2026-09-09/manual-reply-summary.md) הם מקור מצב הבדיקות והפרסום הסופי. סעיף זה אינו טענה ש־CI או הפריסה כבר הסתיימו.

64.9 בביקורת מרוץ נוספת נוספה קשירה ל־Policy event המקורי: כיבוי, החלפה או פקיעת ראיות מדיניות אחרי השמירה מונעים Seal. בדיקת PostgreSQL חשפה שכותב מדיניות קיים לא לקח את Tenant barrier שנדרש במיגרציה 0057; נעילת החיבור שלו תוקנה עם ה־CTE המשותף. בדיקות ה־Repository ושירות הניהול עברו, ואחריהן 13 בדיקות PostgreSQL. מדיניות ה־Rate Limit עצמה וערכי הספק לא שונו. ההכרעה R168.

64.10 בהרצה המלאה השנייה עברו שתי הבניות ו־4,472 מתוך 4,473 בדיקות. נותר משפט במרשם ההגירה שעדיין מנה 44 מיגרציות ליעד בלבד במקום 45; תוקן המרשם. לוג ההרצה נשמר, והרצה לאחר תיקון המדיניות והמרשם נדרשת בדוח הסופי.

# 65. תיקון שילוב Inbox — 10.09.2026

65.1 במסגרת משימות 5.2/7.1 נבדקו ההשלכות של G04 על יתר פעולות ה־Inbox. נמצא כי הסתרת שיחה בסינון מסירה את ה־Composer יחד עם בקשה שהתגובה אליה אבדה; חזרה לשיחה עלולה להתחיל כוונה חדשה בגרסה חדשה. מצב הטיוטות הועבר ל־Inbox ונשמר לפי conversationKey, גם כשאין שיחה נבחרת.

65.2 נמצא גם ש־Polling בודק רק רענון פעיל, בעוד פעולות שיוך, סימון קריאה וטעינת שיחה לא החזיקו את אותו מנגנון. רענון יכול היה לקרוא מידע לפני השינוי ולהחיל אותו אחרי הצלחתו. כעת כל קריאה ופעולה בממשק תופסות מנגנון משותף באופן סינכרוני. פעולה חופפת אינה מתחילה, ו־finally משחרר את התפיסה גם לאחר שגיאה.

65.3 מענה ידני מחזיק את התפיסה עד קריאת האישור שאחרי השמירה. אובדן תגובת השמירה משמר את Payload המקורי; דחייה ודאית ראשונה מאפשרת תיקון הטקסט. כשל בקריאה שאחרי אישור השמירה אינו חוזר על השליחה. מנגנון התפיסה אינו הרשאה: בדיקות ההרשאות, הגרסאות וה־Outbox בשרת ממשיכות לאכוף את הפעולה.

65.4 ההכרעות R169–R170. נוספו בדיקות התנהגות לבקשות מושהות, לחיצה מקבילה, שינוי גרסת שיחה, שימור טיוטה של שיחה אחרת, תגובה פגומה, כשל בקריאת האישור ושחרור ישן שמגיע שוב. הבדיקות משתמשות בנתוני הבדיקות הקיימים ובמחסומי Promise; אין דפדפן או חשבון ספק חי. [דוח האימות](../../outputs/launch-validation-2026-09-09/inbox-concurrency-validation.json) מתעד את התוצאות בפועל; [סיכום והכרעות](../../outputs/launch-validation-2026-09-09/inbox-concurrency-summary.md).

65.5 המצב נשמר בזיכרון רכיב ה־Inbox בלבד. טעינת עמוד מחדש או יציאה ממנו אינה משמרת טיוטות; לא נוסף מאגר טקסט בדפדפן. נדרשות בדיקות דפדפן של מעבר שיחות, סינון, אובדן רשת ו־RTL ב־Staging. מפת התהליכים ומצב 4.6.2 תוקנו להסרת תיאורים שכבר אינם נכונים.

65.6 נותרו 12 שלבי־על. 30–60 דקות היה אומדן העבודה המקומית לתיקון זה בתחילתו, ואינו אומדן של יתרת התוכנה. קבלה חיה, QR, תשתיות ופריסה עדיין פתוחים, ומועד הסיום הכולל אינו ידוע. לא נוסתה גישה מחדש למחשב שהיה נעול בבדיקה האחרונה.


# 66. תיקון סדר היסטוריה ומדיה ו־CI ל־Coexistence — 10.09.2026

66.1 בדיקת משימות 4.6–4.8 ו־7.1 מצאה שחמש סוויטות PostgreSQL קיימות לא נכללו ב־CI: בקשות סנכרון, קליטת היסטוריה, Inbox/מדיה, מחזור סנכרון ומחזור חיבור. נוסף Job נפרד בשם `meta-coexistence`, עם PostgreSQL מקובע ל־digest וחמישה מסדים מבודדים. הרצה מסרבת לאשכול שאינו ריק או לכתובת שאינה כתובת הבדיקות המדויקת; אין מחיקת מסד, איפוס או fallback ל־DATABASE_URL.

66.2 ההרצה הראשונה חשפה תחרות אמיתית ב־Worker: יצירת הודעות ההיסטוריה ושיוך המדיה רצו כמשימות Maintenance מקבילות. כאשר השיוך התחיל ראשון, הוא לא מצא הודעות מקור והסתיים בלי לשייך מדיה באותו מחזור. כעת שתי הלולאות באותה משימה: תחילה Projection שמסתיים ב־Commit, ואז Binding. גבולות העבודה הקיימים — עד 10 דפים ועד 100 שיוכים במחזור — נשמרו. אלה תקציבי עבודה מקומיים, לא מגבלות ספק.

66.3 בדיקת ה־Scheduler הממשי משחזרת 101 רשומות מדיה, דורשת 100 שיוכים במחזור ואת השיוך הנותר במחזור הבא. לפני התיקון נוצרו אפס שיוכים. המועמדים שנותרו מהכשל גרמו לבדיקות המדיה הבאות לפנות לשיחה הלא נכונה ולקבל AUTHORIZATION_CHANGED. הבדיקה המבודדת ותוצאות הכשל נשמרו; בדיקות ההרשאות לא הוחלשו. לאחר התיקון הבדיקה והתרחישים שבאו אחריה עברו.

66.4 בהרצה המלאה שאחרי התיקון עברו 102 בדיקות בסוויטת Inbox/מדיה לפני שתהליך הבדיקה הגיע לתקציב 120 שניות. תקציב תהליך הבדיקה בלבד הוגדל ל־300 שניות, עם פלט TAP שמציג כל כשל מיד ו־15 דקות לכל Job. זמני ה־Worker, המסד והספק לא שונו. התוצאה הסופית חייבת לכלול את כל הסוויטות; ריצה שנקטעה אינה הצלחה.

66.5 חוזי CI ו־Governance דורשים מעתה עשרה Checks, כולל `meta-coexistence`. ראיית תשעה Checks ישנה אינה מספקת לגרסה החדשה. הגנת הענף ב־GitHub נשארת תנאי נפרד שיש לאמת בפועל; הוספת Workflow אינה הוכחה שהוגדרה הגנה או שה־PR אושר.

66.6 ההכרעות R171–R173. [דוח האימות הסופי](../../outputs/launch-validation-2026-09-09/coexistence-ci-validation.json) ו־[מסמך ההכרעות בצד](../../outputs/launch-validation-2026-09-09/coexistence-ci-summary.md) מתעדים בדיקות, זהות הקוד ופרסום. אין בסעיף זה כשלעצמו טענה ש־CI הסתיים. הבדיקות משתמשות ב־Fixtures הקיימים; הן אינן חיבור Meta, סריקת QR או בדיקת מדיה ב־S3 חי.

66.7 נותרו 12 שלבי־על. 45–90 דקות היה אומדן מקומי בתחילת עבודה זו ואינו זמן לסיום המוצר. אימות סנכרון מלא, חידוש חיבור, UI וחשבונות ב־Staging נשאר פתוח; מועד סיום כולל אינו ידוע. לא נוסתה גישה חוזרת למחשב הנעול.

66.8 ההרצה המקומית המלאה עברה: 244 בדיקות PostgreSQL בחמש הסוויטות (11 + 16 + 185 + 15 + 17), ללא כשל או דילוג. עברו גם שתי בניות ו־4,484 בדיקות יחידה, TypeScript, ESLint וגבולות הקוד והסודות. בדיקות ממוקדות נוספות דוחות במפורש ראיית תשעה Checks שחסר בה Coexistence. קריאת GitHub API בזמן עבודה זו אישרה ש־main עדיין אינו מוגן; אין ראיית Governance מלאה.

66.9 ב־GitHub Run 34486919601 עברו תשעה מתוך עשרה Checks, כולל שתי הבניות ו־4,484 בדיקות. Coexistence נעצר: 112 בדיקות Inbox/מדיה עברו ללא כשל, ואז פג תקציב 300 השניות. זמני התרחישים הכבדים היו בערך פי שניים מהמקומי (50.7 לעומת 103.4 שניות בתרחיש Discovery; 9.9 לעומת 19.6 שניות בתרחיש Scheduler). R174 מגדיל ל־600 שניות רק את תקציב תהליך סוויטת Inbox/מדיה; יתר הסוויטות נשארות ב־300 שניות וה־Job ב־15 דקות. נוסף דיווח exit/signal לסוויטה שלא הסתיימה בהצלחה. זו התאמת בדיקות ל־Runner שנמדד; אין שינוי בזמני ייצור או ויתור על תרחיש. נדרשת הרצת CI מלאה חדשה; הריצה שנקטעה נשמרת ככישלון.


# 67. הגנת main ובידוד תרחישי Coexistence — 10.09.2026

67.1 הושלמה הפעלת הגנת main במסגרת משימה 2.1. קריאות API אישרו תחילה שאין הגנה או Rulesets, ושיש גישת Admin. לאחר הפעולה קריאה חוזרת אישרה את עשרת ה־Checks עם app_id=15368 שנקרא מ־GitHub Actions בפועל, strict, אכיפה גם על מנהלים, Review אחד, ביטול אישורים ישנים, Code Owner review, פתרון שיחות וחסימת Force Push ומחיקה. לא נוצרו Collaborators ולא בוצעו Review, Merge או פריסה.

67.2 קובץ CODEOWNERS עדיין חסר במועמד הנוכחי, ולא הושלמה ביקורת PR. לכן זו סגירת פער הגנת הענף בלבד; אין ראיית Governance מלאה ואין סגירת שלב 2 כולו. R175 מתועדת ב־[דוח הגנת main](../../outputs/launch-validation-2026-09-09/governance-main-validation.json). [חוזה GitHub הרשמי](https://docs.github.com/en/rest/branches/branch-protection#update-branch-protection) נבדק. בקשת API ראשונה נדחתה ב־422 בשל שילוב contexts ו־checks; הוסר השדה הישן וההגדרה אומתה בלי שינוי במדיניות.

67.3 Commit ‏1984c0d פורסם לאחר אישור מפורש. בריצת GitHub ‏34488787962 עברו תשעה מתוך עשרה Checks. תקציב 600 השניות הספיק: כל 185 תרחישי המדיה רצו ב־430.78 שניות, אך אחד נכשל (184 עברו). בדיקת ביטול משימה ראתה pending במקום cancelled. ארבע סוויטות Coexistence האחרות עברו; אין לסמן את ההרצה כ־PASS.

67.4 בתשתית הבדיקות נמצא שניקוי התור טיפל במשימות שכבר הגיע זמנן בלבד. Retry עתידי יכול להישאר מבדיקה קודמת ולהיות זכאי לתביעה בזמן הכנת Fixture מאוחר יותר. האיטיות ב־CI חשפה תלות בזמן. R176 מבודדת נתונים בין תרחישים: המסד הייעודי נבדק כריק, מוחלות עליו המיגרציות ונוצר ממנו מסד תבנית בבעלות הבדיקה. אחרי כל תרחיש נסגרות ההתחברויות ומסד ה־Fixtures מוחלף בעותק התבנית. מסד תבנית קיים מראש נדחה; אין ניתוק כפוי של לקוח זר. ניסיון ניקוי קודם ב־TRUNCATE נדחה ב־Trigger ונשמר ככישלון; ההגנות לא בוטלו. התבנית משמרת גם נתוני Seed אם קיימים. אין שינוי בקוד מוצר או החלשת בדיקות הרשאה. בדיקת הביטול דורשת כעת גם זהות Tenant נכונה ואישור הצלחת Finish.

67.5 [דוח האימות](../../outputs/launch-validation-2026-09-09/coexistence-ci-validation.json) ו־[הסיכום בצד](../../outputs/launch-validation-2026-09-09/coexistence-ci-summary.md) מתעדים את התוצאה הסופית ואת ה־Commit שנבדק. נותרו 12 שלבי־על. 10–20 דקות הוא אומדן ראשוני לתיקון הבידוד ואימותו המקומי; נדרשת אחריו ריצת CI. זמן הסיום הכולל אינו ידוע, והקבלה החיה עדיין מחייבת גישה לסביבות ולמכשיר.

67.6 ההרצה המקומית אחרי בידוד באמצעות מסד תבנית עברה במלואה: 244 בדיקות PostgreSQL, כולל 185/185 תרחישי המדיה, ללא כשל או דילוג. סוויטת המדיה ארכה 131.722 שניות. עברו גם 26 בדיקות ממוקדות, ESLint וגבולות המקור. הניסיון הקודם עם TRUNCATE נשמר ככישלון ואינו חלק מהתיקון הסופי. נדרשת ריצת CI ל־Commit התיקון; תוצאות הריצות הקודמות נשארות מקושרות לגרסאות שנבדקו בפועל.


# 68. מסלול שחרור עם בעלים יחיד — 10.09.2026

68.1 Commit ‏406fa29 פורסם ועבר את עשרת Checks ב־GitHub: 4,484 בדיקות יחידה, שתי הבניות, 244 בדיקות Coexistence ועוד 36 בדיקות PostgreSQL/Redis. הופקה ראיית CI הקשורה ל־Commit ול־Release ID. [דוח הגרסה](../../outputs/launch-validation-2026-09-09/coexistence-ci-validation.json). קבלה חיה ופריסה טרם בוצעו.

68.2 בהמשך משימה 2.1 נמצאה סתירה: ADR-0007 מגדיר את טל כאחראי היחיד, אך R175 ו־Evidence v4 דרשו מבקר נוסף. GitHub אינו מאפשר למחבר לאשר את ה־PR שלו. הדרישה למבקר נוסף שהוצגה לטל מבוטלת במסלול בעלים יחיד; אין צורך להמציא משתמש או להקצות אחריות לאדם אחר. R177 מחליפה את רכיב הביקורת של R175.

68.3 נבחר PR מוגן עם אפס Approvals מחייבים, ללא Code Owner approval או Last push approval. חובת PR, כל עשרת Checks הקשורים ל־App שנצפה, Strict, אכיפה על מנהלים, פתרון שיחות, ביטול אישורים ישנים, חסימת Force Push/מחיקה והגנות הסודות נשמרים. CODEOWNERS מצהיר בעלות אמיתית ואינו מציג ביקורת עצמאית שלא התקיימה. [חוזה GitHub הרשמי](https://docs.github.com/en/rest/branches/branch-protection#update-branch-protection) תומך באפס Reviewers.

68.4 חוזה Evidence v5 מציין reviewPolicy=single-owner, דורש חשבון User של טל עם הרשאות Admin/Push כרשומת Collaborator יחידה, ובודק תוכן, גודל, Base64 ו־Git blob SHA של CODEOWNERS. הוא דוחה Metadata בלבד, בעלים אחר, כללי Override, Bypass, App לא מחייב או ראיית v4. אין שינוי במיגרציות, בנתוני הלקוחות, במגבלות Meta או במסלול השליחה.

68.5 האימות המקומי, קריאת הגנת הענף, זהות ה־Commit והפרסום יתועדו ב־[דוח האימות](../../outputs/launch-validation-2026-09-09/single-owner-validation.json) וב־[מסמך ההכרעה בצד](../../outputs/launch-validation-2026-09-09/single-owner-summary.md). קיום הקוד אינו טענה שההגדרה נשמרה בחשבון או ש־Governance Evidence הופק. בדיקת CI של 406fa29 נשארת מקושרת לגרסה זו בלבד.

68.6 נותרו 12 שלבים ראשיים. אומדן ראשוני לסבב התיקון והאימות: 20–40 דקות; זמן הסיום הכולל אינו ידוע. אישור פתיחה ידנית של ה־Mac וחשבונות הספק עדיין חסרים לפריסה ול־QR. בקשת זהות המבקר הנוסף הקודמת אינה תנאי להמשך במסלול הנוכחי.


68.7 **מצב היסטורי לפני האישור המפורש:** האימות המקומי עבר: שתי הבניות וכל 4,490 בדיקות היחידה, 31 בדיקות ממוקדות, TypeScript, ESLint, Source guardrails וסריקת סודות. קריאת GitHub אישרה בעל הרשאות יחיד ועשרה Checks מחייבים. ניסיון התאמת ביקורות main נדחה בבדיקת האישור האוטומטית לפני ביצוע. באותו שלב נדרש אישור מפורש לשינוי חובת הביקורת ולא בוצעו שינוי המדיניות או המיזוג. החסם נפתר בהמשך, כמפורט בסעיף 68.8.

68.8 **R177–R180 בוצעו לאחר האישורים המפורשים:** מדיניות הבעלים היחיד הוחלה; בסיס הענף עודכן מעשרת המסמכים שכבר היו ב־main; [PR #2](https://github.com/talstilkol/connect/pull/2) מוזג ל־main ‏4ee61e05f18eece0c4ef85e2a27f035972a7b3d0. כל עשרת Checks עברו על Commit המיזוג עצמו: 4,490 בדיקות יחידה, שתי הבניות, 244 בדיקות Coexistence ועוד 36 PostgreSQL/Redis. ביקורות Production ו־Development מצאו אפס חולשות; חתימת ביקורת התלויות אומתה. Governance v5 ו־CI הופקו מחדש ל־main. [Quality](https://github.com/talstilkol/connect/actions/runs/34499646377), [Audit](https://github.com/talstilkol/connect/actions/runs/34499651584), [דוח הסגירה והחסמים שנותרו](../../outputs/launch-validation-2026-09-09/main-merge-summary.md). אלה ראיות לגרסה זו; אין Deployment או QR חי.

# 69. התאמת Preflight לתכונות המוצר — 10.09.2026

69.1 במשימה 2.4 נמצא פער: בודק תצורת הפיילוט לא בדק את מתגי הגשת/סנכרון Templates, הפעלת קמפיין ומענה ידני שה־Runtime כבר דורש. ערך הפעלה שגוי או מפתח חסר במסלול מופעל לא נכללו בדוח, אף ש־Startup היה דוחה אותם.

69.2 R181 מחברת את הבודק ל־Validators הקיימים. API בודק את ארבעת המסלולים; Worker בודק מענה ידני; Web נשאר עם חוזה הזהות וה־API שלו. כבוי מוצג disabled ומותר בהכנה; invalid חוסם. לא נוסף מפתח חדש, לא הופעל מתג ולא שונה Runtime של שליחה.

69.3 שלוש בדיקות רגרסיה נכשלו לפני התיקון ועברו אחריו. 35 בדיקות ממוקדות עברו, עם TypeScript, Lint ו־Source guardrails. נבדקו ערכי הפעלה פגומים, תנאים חסרים, ההבדל בין Boolean/String/Object שהבודקים מחזירים, שיוך הבדיקה לשירות ואי־חשיפת ערכים. נעשה שימוש בקלטי הפרוטוקול שכבר קיימים בבדיקות Startup; אין חשבון או מכשיר מדומים שמוצגים כראיית ספק.

69.4 [מדריך ההפעלה](../pilot-deployment-runbook.md), [דוח האימות והפרסום](../../outputs/launch-validation-2026-09-09/pilot-feature-preflight-validation.json), [סיכום בצד](../../outputs/launch-validation-2026-09-09/pilot-feature-preflight-summary.md). תוצאות CI של main בסעיף 68.8 אינן ראיה לתיקון זה; מצב האימות והפרסום שלו נקשר ל־Commit שלו בדוח.

69.5 נותרו 12 שלבים ראשיים. אומדן סבב ההרחבה והאימות 15–30 דקות; אומדן הסיום הכולל עדיין אינו ידוע. תצורה מקומית תקינה אינה קבלת מוצר: נדרשים Staging, זהות ושירותים חיים, ובריאות Worker וחיבור QR/קוד במכשיר Business מורשה. פתיחה ידנית של ה־Mac עדיין לא אושרה ולא חודשה אוטומציית המחשב שנחסמה.


## 70. עריכות כיתוב מדיה ב־Business App — 10.09.2026

70.1 פער ממשי במשימות 4.7 ו־W09: `smb_message_echoes` עם `edit.message.type=image` נדחה לפני כתיבה, אף שזהו מבנה שמתועד אצל Meta. קריאה ישירה של [התיעוד הרשמי](https://developers.facebook.com/documentation/business-messaging/whatsapp/webhooks/reference/smb_message_echoes/) הצליחה ב־10.09.2026 לאחר שכלי החיפוש החזיר שגיאות גישה. אין שינוי במגבלות שליחה או בחוזה הרשמה.

70.2 נוספה תמיכה בעריכת כיתוב לתמונה, וידאו ומסמך, לרבות הסרת כיתוב. סוג המדיה נשמר בשיחה ובתצוגה המקדימה. מיגרציה 0074 מוסיפה `edit_kind` למצב העריכה העמיד ומשמרת עריכות טקסט ואישורי אירועים קיימים. הקלט אינו מחליף קובץ או הופך כתובת ספק להורדה מורשית. נשמרים בידוד Tenant ונכסים, סדר זמנים, מניעת Replay, עצירה בסתירה באותה שנייה ומחיקה סופית.

70.3 עריכה לפני המקור שומרת גם את סוג התוכן; מקור מסוג אחר נדחה. בהיסטוריה שכבר מכילה סוג מדיה מזוהה, הכיתוב מוצג רק כשסוג העריכה תואם למקור. `media_placeholder` וחידוש גישה לקובץ לאחר עריכה עדיין דורשים טיפול נפרד; אין הבטחה שמדיה מלאה הושלמה. כיתוב מקורי של Echo נשאר בהתנהגות הקיימת כדי לשמר Digest של הודעות ואישורים ישנים; שינוי חוזה זה נשאר משימת המשך מפורשת.

70.4 בדיקות השינוי כוללות רגרסיה שנכשלה לפני המימוש, DTO ותצוגה בשלוש שפות, שדרוג נתוני עריכה מלפני 0074, כל סדרי מקור/עריכה/מחיקה, כיתוב ריק, סוג שגוי, Replay ומקביליות. סוויטת Echo הקיימת נוספה להרצת Coexistence ב־CI: שש סוויטות במסדים נפרדים. [דוח התוצאות והגרסה](../../outputs/launch-validation-2026-09-09/meta-media-caption-validation.json), [סיכום בצד](../../outputs/launch-validation-2026-09-09/meta-media-caption-summary.md).

70.5 סדר פריסה: להשהות Worker שמעבד Webhooks, להחיל 75 מיגרציות PostgreSQL, לפרוס API ו־Web שקוראים כיתובים ערוכים, ולחדש Worker תואם. אין להריץ קוראי Inbox ישנים לצד כותב הכיתובים החדש. לפני חידוש נדרשים אימות בריאות וזהות הגרסה. אין כאן פריסה חיה או הוכחת QR.

70.6 נותרו **12 שלבים ראשיים**. זו השלמת חלק מעריכות המדיה, ולא סגירת שלב 4. המימוש קודם בזמן חסם הגישה לסביבות בעקבות הוראת ההמשך; Staging וקבלה חיה נשארים בעדיפות במסלול ההשקה. אומדן הסבב הראשוני 45–90 דקות אינו אומדן לסיום המוצר; זמן הסיום הכולל עדיין אינו ידוע. ההכרעות R182–R184 במסמך ההחלטות.

## 71. בדיקות הרשמה והמשך Coexistence בשער החובה — 10.09.2026

71.1 בבדיקת מסלול 4.5–4.6 נמצא ששתי סוויטות PostgreSQL קיימות לא נכללו בהרצת CI: `meta-signup-launch-postgres` ו־`meta-coexistence-orchestration-postgres`. הן כוללות 70 בדיקות של הכנת הרשמה, החלפת קוד חד־פעמית, הרשאות, שמירת תוצאה, המשך עבודה ב־Worker וסדר בקשות הסנכרון. בדיקות כלליות ובדיקת QR חיה אינן תחליף לבדיקות המסד האלה.

71.2 שתי הסוויטות נוספו ל־`meta-coexistence`, שהוא כבר בדיקת חובה. ההרצה כוללת כעת שמונה מסדים מבודדים, עם כתובות Loopback קבועות וללא שימוש ב־DATABASE_URL, מחיקה או שימוש חוזר במסד קיים. משתנה החיבור של סוויטת התזמור נקשר למסד שלה בתהליך הבן בלבד; כתובת מסד הבקרה נשארת נפרדת. מספר בדיקות החובה נשאר עשר.

71.3 נוספה בדיקה שמשווה את רשימת סוויטות Meta PostgreSQL בתיקיית Integration לרשימת ההרצה ב־CI, ומוודאת שאין שיתוף של שם מסד או משתנה חיבור בין סוויטות. היא נכשלה לפני התיקון בגלל שתי הסוויטות החסרות. תוספת סוויטה בעתיד מחייבת רישום מפורש בשער.

71.4 זהו שינוי בכיסוי בדיקות, ללא שינוי בהתנהגות המוצר, בסכמה, במכסות או בתצורת ספק. התוצאות והגרסה הסופיות נשמרות ב־[דוח האימות](../../outputs/launch-validation-2026-09-09/signup-ci-validation.json) וב־[סיכום בצד](../../outputs/launch-validation-2026-09-09/signup-ci-summary.md). ההכרעות R185–R186 במסמך ההחלטות.

71.5 נותרו **12 שלבים ראשיים**. בדיקת התהליך הנוכחי עדיין מצאה תצורה חסרה ל־Web/API/Worker; זו אינה בדיקה של חשבונות הספקים. נדרשות גישה לסביבות ובדיקת QR/קוד אמיתית לפי משימה 4.9. אומדן הסבב הראשוני 20–40 דקות מתייחס להרחבת CI בלבד; זמן ההשקה הכולל עדיין אינו ידוע.

## 72. סגירת חלון ההרשמה מבטלת ניסיון ממתין — 10.09.2026

72.1 בבדיקת משימה 4.8 שוחזר כשל: אחרי קבלת אירוע סיום מ־Meta, סגירת חלון Connect הסירה את מאזין ההודעות אך השאירה את מתאם הניסיון פעיל. תשובת SDK מאוחרת עם קוד הרשאה יכלה להפעיל את בקשת ההשלמה בשרת לאחר הסגירה. השחזור נעשה עם EventTarget ותרחישי הבדיקה הקיימים, ללא פנייה ל־Meta או שימוש בדפדפן.

72.2 נוסף ביטול שקט של מתאם הניסיון: הוא מסומן כסגור וקוד ההרשאה והנכסים שהמתינו בזיכרון משוחררים. כפתורי הסגירה, הרקע ו־Escape מבטלים את הניסיון לפני הקריאה לסגירת החלון; גם הסרת הרכיב מפעילה את אותו ניקוי. ניסיון חדש משתמש במתאם חדש.

72.3 לפני טיפול בתשובת SDK או השלמה נבדקים החלון הפעיל וזהות הניסיון הנוכחי. תשובה מאוחרת לניסיון סגור אינה מגישה בקשה ואינה פותחת Timer חדש. סיום תקין עדיין מגיש פעם אחת, בשני סדרי הגעת הקוד והנכסים ובשני מסלולי ההרשמה. ביטול מקומי אינו מבטל בקשה שכבר הוגשה לשרת או חיבור שכבר הושלם אצל Meta.

72.4 נוספו בדיקות ביטול לפני ואחרי כל תוצאה חלקית, ניקוי חוזר, תשובת SDK מאוחרת לצד ניסיון חדש, וסיום תקין עם ניקוי סינכרוני. [דוח האימות והגרסה](../../outputs/launch-validation-2026-09-09/signup-cancellation-validation.json), [סיכום בצד](../../outputs/launch-validation-2026-09-09/signup-cancellation-summary.md). ההכרעות R187–R188 במסמך ההחלטות.

72.5 נותרו **12 שלבים ראשיים**. זהו תיקון בהתנהגות מסך החיבור, ולא הוכחת QR או השלמת שלב 4. אין שינוי במסד, בזרימת הספק, בתצורת ההפעלה או במכסות. אומדן הסבב הראשוני 25–45 דקות מתייחס לתיקון ולבדיקות בלבד; זמן ההשקה הכולל עדיין אינו ידוע ותלוי ב־Staging ובקבלה חיה.

## 73. הצגת כיתובים מקוריים מהיסטוריית WhatsApp — 10.09.2026

73.1 במשימות 4.6–4.7 ו־W09 נמצא כיתוב שנשמר באצוות ההיסטוריה אך הוסתר בקריאה: SQL החזיר טקסט רק להודעת טקסט או לעריכת Echo, והתצוגה הציגה כיתוב מדיה רק במצב edited. שתי בדיקות PostgreSQL ובדיקת תצוגה שיחזרו את הפער לפני התיקון. [תיעוד Meta הרשמי](https://developers.facebook.com/documentation/business-messaging/whatsapp/webhooks/reference/history/) נקרא ישירות ב־10.09.2026 ומאשר כיתוב אופציונלי בתוכן ההיסטוריה; פתיחה דרך כלי החיפוש החזירה תחילה 429.

73.2 קורא Inbox מציג כעת caption שהוא String בתמונה, וידאו או מסמך מהאצווה המקורית, בשיחה ובתצוגה המקדימה, בעברית, אנגלית וערבית. כיתוב חסר או שאינו String אינו הופך לטקסט. כיתוב ריק או רווחים נשמר כפי שנקלט אך אינו מוסיף טקסט לתצוגה. אין מיגרציה, העתקת כיתוב למסד נוסף או שינוי Digest/Receipt של אירוע קיים.

73.3 עריכה מאוחרת, הסרת כיתוב, סתירה ומחיקה ממשיכות לגבור על הכיתוב המקורי. הרשאת ההיסטוריה, מצב החיבור, Tenant והסכמת השיתוף נשארים תנאי קריאה. הצגת כיתוב אינה אישור קובץ, אינה יוצרת הודעה חיה או Unread ואינה פותחת חלון שירות. כיתובים מקוריים של Echo חי, Media placeholders וחידוש הרשאת קובץ אחרי עריכה נשארים משימות נפרדות.

73.4 הבדיקות כוללות שני כיווני הודעה ושלושה סוגי מדיה, Round-trip של DTO, שימור אצווה ו־Digest, סירוב שיתוף אחרי הצגה, כיתוב חסר/ריק/פגום ואי־הצגת כיתוב בסוג אחר. [דוח האימות והפרסום](../../outputs/launch-validation-2026-09-09/history-original-caption-validation.json) ו־[סיכום בצד](../../outputs/launch-validation-2026-09-09/history-original-caption-summary.md) מתעדים תוצאות בפועל. ההכרעות R189–R190.

73.5 נותרו **12 שלבים ראשיים**. אומדן הסבב הראשוני 30–60 דקות כולל תיקון, בדיקות ופרסום, ואינו אומדן להשקת המוצר. יש לפרוס API ו־Web מאותה גרסה כדי שהקורא והתצוגה יתמכו בכיתוב. אין הפעלה חיה, בדיקת QR או קבלת Staging בסבב זה; זמן סיום כולל עדיין אינו ידוע.

## 74. הצגת מדיה שקושרה להודעת היסטוריה — 10.09.2026

74.1 במשימות 4.6–4.7 ו־W09/W10 נמצא שקישור מדיה מאומת אינו מתבטא ב־Inbox: המקור נשאר `media_placeholder` גם אחרי Binding. ארבע בדיקות PostgreSQL שיחזרו את התקלה לפני השינוי; בדיקה נוספת אישרה שעריכה ללא סוג מקור מוכח נשארת חסומה. נעשה שימוש חוזר בתיעוד [History הרשמי של Meta](https://developers.facebook.com/documentation/business-messaging/whatsapp/webhooks/reference/history/), שנשמר בקריאה ישירה ב־10.09.2026 בסבב הקודם: תוכן המדיה יכול להגיע באירוע היסטוריה נפרד מההודעה המקורית.

74.2 קורא Inbox משלים סוג מדיה וכיתוב אופציונלי מתוך Binding קיים בלבד, בהתאמה ל־Tenant, מזהה ההודעה וה־Digests של שני המקורות. ההרשאה לסשן, לחיבור הנוכחי, לבקשת ההיסטוריה ולאצווה נבדקת בכל קריאה. כיתוב String מוצג בתמונה, וידאו ומסמך; Audio ו־Sticker מציגים את סוגם. מקור שכבר מכיל סוג מדיה שומר את התיאור והכיתוב שלו. ללא Binding ההודעה נשארת Placeholder.

74.3 עריכת כיתוב יכולה כעת להתאים לסוג המקור שהוכח ב־Binding. עריכה, סתירה ומחיקה גוברות על תוכן המדיה; סוג סותר מוסתר. הקישור אינו מתיר הורדת קובץ לאחר עריכה. עריכה שהגיעה לפני Binding עדיין מונעת את הקישור במסגרת הכללים הקיימים, ולכן הצגת מקרה זה נשארת להמשך. לא שונו המקורות, האצוות, ה־Digests, ה־Receipts, זמני ההודעה, הכיוון, מצב המסירה או חלון השירות; אין מיגרציה חדשה.

74.4 נוספו חמישה מבחני אינטגרציה, הכוללים חמישה סוגי מדיה, שני סדרי הגעה, שני כיווני הודעה, הפרדה בין לקוחות, DTO ותצוגה מקדימה, שימור מקור, הסרת כיתוב, סתירת עריכות, מחיקה, סירוב שיתוף, סתירת מדיה וביטול הרשאת החיבור. שתי ציפיות קיימות עודכנו כך שסוג מדיה מאומת מוצג גם בלי הוכחת זמינות קובץ. [דוח האימות והפרסום](../../outputs/launch-validation-2026-09-09/history-bound-media-validation.json) ו־[מסמך ההכרעות בצד](../../outputs/launch-validation-2026-09-09/history-bound-media-summary.md) מתעדים תוצאות בפועל. ההכרעות R191–R192.

74.5 נותרו **12 שלבים ראשיים**. אומדן הסבב הראשוני 30–60 דקות מתייחס לתיקון, בדיקות ופרסום בלבד. כיתוב מקורי של Echo חי, עריכה לפני Binding, חידוש הרשאת קובץ, השלמת ייבוא וחידוש חיבור נשארים פתוחים. נדרשים API/Web תואמים וכן קבלה חיה ב־Staging, כולל QR/קוד במכשיר מורשה; אין קבלת ספק או פריסה בסבב זה. זמן ההשקה הכולל עדיין אינו ידוע.

## 75. קישור מדיה אחרי הגעת עריכת כיתוב — 10.09.2026

75.1 כהמשך למשימות 4.6–4.7, נמצא שמסנן המקורות של קישור המדיה משותף להרשאת ההורדה: כל עריכה קודמת מנעה Binding והשאירה Placeholder ערוך מוסתר. חמישה מתוך ששת מבחני הרגרסיה החדשים נכשלו לפני השינוי; מבחן ההרשאות השלילי עבר. [חוזה History של Meta](https://developers.facebook.com/documentation/business-messaging/whatsapp/webhooks/reference/history/), שנשמר בקריאה ישירה ב־10.09.2026 ונקרא שוב בסבב זה, מפריד את אירוע המדיה מההודעה המקורית. אין להניח שהקישור המקומי יסתיים לפני הגעת עריכת Echo.

75.2 תנאי ההרשאה המשותפים נשמרים; רק בחירת מקור ל־Binding מאפשרת גם עריכת כיתוב או סתירת עריכות באותו סוג תמונה, וידאו או מסמך. התנאי נבדק גם בעת בחירת מועמד וגם לאחר המתנה לנעילת סשן ההיסטוריה. נדרשים Tenant, חיבור ונכסים נוכחיים, בקשת היסטוריה והסכמה תקפות, נמען תואם ומקורות לא סותרים. המקורות מנורמלים וה־Digests שלהם נבדקים לפני קישור בלתי־משתנה; Binding ו־Audit נכתבים באותה טרנזקציה.

75.3 קריאות לצורכי הורדה ממשיכות לדרוש מצב מקור original. עריכת כיתוב מוקדמת אינה מאשרת קריאת קובץ, גישת ספק או שימוש בכיתוב כדי לזהות Bytes. הקורא שכבר מומש בסעיף 74 מציג את העריכה לאחר הקישור; הסרת כיתוב וסתירה אינן מחזירות כיתוב ישן. מחיקה נשארת סופית, וסוג מדיה שגוי אינו מקבל קישור. אין שינוי בסכמה, במקורות שנקלטו, בזמני ההודעה, ב־Unread או בחלון השירות.

75.4 שש בדיקות חדשות מכסות את כל ששת סדרי הגעת היסטוריה/מדיה/עריכה בשלושה סוגי מדיה, כיתוב שהוסר, סתירה והמשך עריכה, נמען וסוג שגויים, ביטול הרשאה/הסכמה, סתירת מדיה, Rollback של Audit, חמישה ניסיונות קישור מקבילים, מחיקה בזמן המתנה לנעילה ו־Digest פגום. בדיקת ההורדה דורשת אפס פניות לתעבורת הספק אחרי קישור של הודעה ערוכה. [תוצאות האימות והפרסום](../../outputs/launch-validation-2026-09-09/history-edited-binding-validation.json), [מסמך ההכרעות בצד](../../outputs/launch-validation-2026-09-09/history-edited-binding-summary.md). ההכרעות R193–R194.

75.5 מגבלת סדר ההגעה שצוינה בסעיפים 73–74 הוסרה בקוד. עדיין פתוחים חידוש הרשאת קובץ אחרי עריכה, כיתוב מקורי של Echo חי, השלמת ייבוא, חידוש חיבור וקבלה מול ספק. נדרש Worker מעודכן לקישור וקורא Inbox תואם מסעיף 74. נותרו **12 שלבים ראשיים**; אומדן הסבב הראשוני 30–60 דקות כולל תיקון, בדיקות ופרסום בלבד. תצורת התהליך עדיין חסומה ב־Web/API/Worker; אין בסבב זה פריסה או בדיקת QR, וזמן ההשקה הכולל אינו ידוע.

## 76. כיתוב מקורי של הודעת Business App חיה — 10.09.2026

76.1 כהמשך למשימה 4.6 ול־W09, נוסף כיתוב מקורי לתמונה, וידאו ומסמך מאירוע `smb_message_echoes`. [חוזה Meta הרשמי](https://developers.facebook.com/documentation/business-messaging/whatsapp/webhooks/reference/smb_message_echoes/), שנשמר בקריאה ב־10.09.2026 ונקרא שוב בסבב זה, מגדיר תוכן לפי סוג הודעה וכיתוב אופציונלי. ניסיון הקריאה החוזרת ברשת החזיר 429; ההכרעה נשענת על העותק הרשמי שכבר נשמר. כיתוב ריק או חסר מנורמל ל־null; קלט שאינו String, NUL או מעל 16,384 יחידות UTF-16 נדחה. זהו גבול Connect קיים, לא טענה למגבלת Meta.

76.2 ה־Parser מעביר `originalCaption` נפרד, בעוד `textContent` המקורי למדיה נשאר null בחוזה הקליטה. Repository שומר בדיוק את סדר השדות וה־Digest של v1 ומחשב Digest נוסף לכיתוב שנצפה לראשונה. מיגרציה 0075 מוסיפה `original_caption_digest` אופציונלי ואינה משכתבת Receipts, Digests או טקסט קיימים. הודעה ישנה או שורה מלפני מנגנון ה־Revision אינה מועשרת מתוך Replay: לא ניתן להוכיח את הכיתוב שה־Parser הישן השמיט. הודעה חדשה שהכיתוב שלה נקשר דוחה Replay עם כיתוב שונה או ללא הוכחת הכיתוב, תחת אותה נעילת הודעה וטרנזקציה.

76.3 כיתוב המקור נשמר רק בתוכן ההודעה המוצג; מצב ה־Echo שומר Digest בלבד. עריכה או הסרת כיתוב מחליפות אותו, סתירה מסתירה אותו ומחיקה נשארת סופית בכל סדר הגעה. מקור שמגיע אחרי Revision אינו מחזיר טקסט ישן. קורא PostgreSQL מקבל כיתובים מקוריים מנורמלים רק במדיה יוצאת נתמכת; אין שינוי בקליטת הודעה נכנסת, הסכמה, Unread, זמן ההודעה, מצב מסירה או חלון שירות. אין הפקת הרשאת קובץ מכיתוב, URL או מזהה מדיה.

76.4 תוכנית הסבב ותנאי סגירה:

76.4.1 פענוח ומימוש תואם v1, מיגרציה ושדרוג קורא השיחה — כ־20–30 דקות בתכנון הראשוני.

76.4.2 בדיקות פענוח, קריאה, שדרוג, Webhook חתום, כפילויות ומרוצים; כל שמונה סוויטות Coexistence, שתי בניות ובדיקות כלליות — כ־25–40 דקות בתכנון הראשוני. שתי בדיקות הפענוח ושש בדיקות המסד הראשונות נכשלו מול הקוד הישן. נוספו גם בדיקות לשני כיתובים מתחרים ולגבולות SQL; תוצאות ההרצה המלאה נשמרות בראיות המקושרות.

76.4.3 הכנת חבילת פרסום מדויקת, PR ומיזוג רגיל אחרי עשרת Checks, ועדכון ראיות לשחרור — כ־15–20 דקות עבודה בתכנון, בתוספת המתנה לאישורי פרסום ול־CI לפי הצורך. מצב הפרסום בפועל מופיע ב־[מסמך הסבב](../../outputs/launch-validation-2026-09-09/echo-original-caption-summary.md) וב־[תוצאות האימות](../../outputs/launch-validation-2026-09-09/echo-original-caption-validation.json). ההכרעות R195–R196; אין לסמן פרסום או פריסה כהושלמו ללא ראיה.

76.5 פריסה מתואמת: להשהות עיבוד Echo, להחיל את כל **76 מיגרציות PostgreSQL**, לפרוס API/Web שתומכים בקריאת הכיתוב, ואז לחדש Worker מעודכן. אין לערבב Worker חדש עם קורא ישן שאינו מקבל כיתוב מקורי; חזרה לגרסה קודמת דורשת קורא תואם, בלי למחוק כיתובים או Digests. היקף Railway-only עולה ל־47; כיסוי D1 נשמר. נדרשת בדיקת קבלה חיה של שלושת סוגי המדיה, Replay, עריכה ומחיקה לפני פתיחת המסלול בפיילוט.

76.6 נשארו חידוש הרשאת קובץ אחרי עריכה, השלמת ייבוא וחידוש חיבור, תשתית S3/סריקה וקבלת ספק. נותרו **12 שלבים ראשיים**. אומדן הסבב הראשוני **60–90 דקות** הוא אומדן עבודה מקומית והכנת פרסום, לא מועד השקה. Staging ו־QR חי לא נבדקו בסבב זה; זמן ההשקה הכולל עדיין אינו ידוע.

## 77. גישת ספקים והנחיות פריסה לפי הגרסה — 11.09.2026

77.1 PR #23 מוזג ל־`671f8183f150bc7d1f9887ee5f9d2ec7b9fac136`; כל עשרת Checks עברו ב־PR וב־main. אומתו 4,505 בדיקות כלליות, שתי בניות ו־401 בדיקות PostgreSQL/Redis. ראיות CI/Governance וחתימת ביקורת התלויות תואמות לגרסה. [תוצאות הסבב שהושלם](../../outputs/launch-validation-2026-09-09/echo-original-caption-validation.json). שלב פרסום זה הסתיים; הוא אינו פריסה או בדיקת QR.

77.2 העדיפות היא משימות 2.2–2.3 ו־4.9: חשבונות, Staging וקבלת ספק. בדיקת גישה חדשה מצאה שאין שמות Secrets או Variables במאגר GitHub או בסביבת staging-e2e. בדיקת זהות Vercel לא השלימה אימות בזמן שהוקצב. אלה ממצאים של מסלולי הגישה שנבדקו, לא קביעה שאין חשבונות או שירותים קיימים. נפתח ניסיון התחברות רשמי שאפשר לאשר ממכשיר אחר; הצלחתו תירשם רק לאחר תשובת ספק ואימות זהות. מצב הגישה העדכני נמצא ב־[מסמך הסבב](../../outputs/launch-validation-2026-09-09/staging-access-20260911-summary.md).

77.3 נמצא פער במדריך הפריסה: הוראת ההפעלה הכללית עדיין הפנתה ל־75 מיגרציות אחרי שפורסמה 0075. ההוראה תוקנה להפניה למלאי שב־Release Manifest המאומת. נוסף סדר פריסה של קוראים תואמים לפני חידוש Worker והובהר מעמדם ההיסטורי של סעיפי 0074. [מדריך הפריסה](../pilot-deployment-runbook.md#11-המשך-לstaging-לאחר-מיגרציה-0075--11092026).

77.4 שלבים ותתי־שלבים להמשך המסלול הקיים:

77.4.1 גישה: להשלים אישור חשבון Vercel ולקרוא זהות, צוות ופרויקט; אחריו לבדוק הרשאות Railway, Clerk ו־Meta. אומדן עבודת האימות המקומית: 10–20 דקות, ללא זמן המשתמש או הספק. אין להשלים כניסה בשם המשתמש במסך נעול.

77.4.2 סביבה: לבחור שירותים אמיתיים ומבודדים, לבדוק תוכנית ותקציב, להגדיר API/Worker/PostgreSQL/Redis, זהות ו־Origins. זהו המשך משימות 2.2–2.3; אומדני הבסיס שלהן נשמרים, ואין אומדן יתרה חדש לפני קריאת המצב אצל הספקים.

77.4.3 גרסה: לאמת מחדש ראיות שתוקפן מתאים למועד הפריסה, להחיל מיגרציות לפי ה־Manifest ולפרוס את השירותים באותה גרסה. להוכיח בריאות, הרשאות ותקשורת בין השירותים לפני פתיחת המסלול לפיילוט.

77.4.4 קבלה: לחבר WhatsApp Business זכאי במסלול Meta, לבדוק QR/קוד, ביטול ופקיעה, הודעה נכנסת/יוצאת/Echo וסנכרון בהסכמה. אומדן 4–8 השעות של משימה 4.9 הוא בסיס היסטורי; אינו התחייבות לפני אימות החשבון והמכשיר.

77.5 נותרו **12 שלבים ראשיים**. אין אומדן אמין להשקה לפני גישה ומיפוי מצב הסביבות. R197–R198 מתועדות במסמך ההכרעות. אין בסבב זה הרחבת תכולת מדיה או שינוי תנאי הקבלה; ממצאי קוד חדשים יתועדפו לפי השפעתם על הפיילוט.

77.6 **המשך אימות הגישה:** ניסיון Vercel הראשון הסתיים ללא הוכחת כניסה; נפתח ניסיון שני. מצב הסבב מפורט ב־[דוח ההמשך](../../outputs/launch-validation-2026-09-09/staging-access-20260911-followup.md). אישור כללי להמשיך בפיתוח אינו מבצע את האימות בחשבון הספק.

77.7 **הכרעות תשתית R199–R200:** מחקר רשמי אישר מסלול מתאים למיגרציות בתוך הרשת הפרטית, ותמיכה ב־Hostname פרטי ביצירת תעודת PostgreSQL. נבחר מבצע מיגרציות חד־פעמי ומבודד עם הרשאה נפרדת, בלי להעניק הרשאות סכימה ל־API/Worker. נשמר אימות TLS מלא עם CA ותעודה שייבדקו בשירות האמיתי. Redis נשאר פרטי עם הגדרות העמידות הקיימות. [פירוט ומקורות](../pilot-deployment-runbook.md#12-הכרעות-חיבור-ומיגרציות-railway--11092026).

77.8 **תת־שלב ביצוע פתוח:** לאחר זיהוי מסד Staging והרשאותיו, להכין ולאמת מבצע מיגרציות שמתאים למצבו ול־Release. כלי תרגיל האתחול המקומי אינו מממש זאת. אין אומדן יתרה אמין למבצע לפני שיובהר אם המסד חדש או קיים ומהן הרשאותיו; המשימה נכללת בשלב 3 ואינה מוסיפה שלב ראשי. נותרו **12 שלבים ראשיים**; פריסה וקבלת QR לא הושלמו.

## 78. חבילת אתחול למסד Staging חדש — 11.09.2026

78.1 כדי לקדם את הכנת הפריסה לפני התחברות לספק, נבחר והוכן מסלול אתחול למסד ייעודי חדש בשם `connect_staging`. החבילה קשורה ל־Release המאומת 671f8183 ומכילה את כל 76 מיגרציות PostgreSQL ואת ה־Hashes שלהן. אין דיווח שמסד היעד כבר קיים בחשבון. [חבילת הביצוע המקומית](../../outputs/launch-validation-2026-09-09/postgres-bootstrap-671f8183/README.md).

78.2 החבילה דורשת את Login המיגרציה ואת הבעלים הקנוניים, בודקת הרשאות והיעדר סכמה קיימת, מחילה נעילה ומבצעת את כל הפעולה בטרנזקציה אחת. ברירת המחדל של הפעלת פונקציות בידי PUBLIC מבוטלת לפני יצירתן. מסד חלקי אינו מאופס. אובדן תשובת Commit דורש בירור מצב לפני פעולה נוספת; אין Retry אוטומטי. הגנות אלה אינן ראיית זהות ספק או אישור Runtime.

78.3 **אימות שהושלם:** 13 בדיקות עברו מול PostgreSQL 17.11 במכל מקומי ללא רשת. כשל לאחר המיגרציה האחרונה וניתוק חיצוני ביטלו את כל הסכמה; מתחרה נחסם; ניסיון אחרי ביטול הצליח; הרצה חוזרת אחרי Commit נדחתה ללא שינוי. אומתו בעלות תפקיד המיגרציה והיעדר EXECUTE ציבורי לפונקציות. [תוצאות](../../outputs/launch-validation-2026-09-09/postgres-bootstrap-671f8183/local-validation.json). כלי בדיקת הניתוק תוקן לאחר שהפרדת התפקידים מנעה סיום עצמי; הרשאות החבילה לא הורחבו.

78.4 **שלב 3, יתרת הביצוע:** לאמת חשבונות ומשאבים → להכין מסד חדש ותפקידים → לאמת TLS ותעודה → לארוז את החבילה במבצע חד־פעמי באותה רשת → לאמת את ה־Release וה־Hashes ולבצע → לאמת הרשאות Runtime ושירותים. מסלול עדכון למסד קיים נשאר נפרד ואינו מכוסה בחבילת האתחול.

78.5 שני ניסיונות Vercel הסתיימו ללא הוכחת התחברות. לא בוצעה פריסה או קבלת QR. הושלמה הכנת החבילה המקומית בלבד; נותרו **12 שלבים ראשיים**. אין אומדן אמין להשקה לפני גישה לספקים. ההכרעות R201–R202 נוספו למסמך ההחלטות.

## 79. אריזת מבצע המיגרציות ל־AMD64 — 11.09.2026

79.1 נבנתה תמונת Linux AMD64 מקומית למבצע האתחול, מבסיס PostgreSQL 17.11 רשמי המקובע ל־Digest. ה־SQL זהה לחבילה מסעיף 78. נוספו סקריפט הפעלה ותצורת Railway ייעודית; לא הועלתה תמונה ל־Registry ולא שונה קוד המוצר. [תוצרי האריזה וההוראות](../../outputs/launch-validation-2026-09-09/postgres-bootstrap-runner-671f8183/README.md).

79.2 המבצע חוסם root, יעד שאינו מתאים, הפחתת TLS, קבצי חיבור חסרים או לא פרטיים ו־SQL ששונה. psql מקבל רק משתנים מורשים וקורא סיסמה מקובץ חיצוני. תצורת השירות כוללת מופע אחד, ללא Cron או HTTP Healthcheck, וללא Restart אוטומטי. ההגדרות מחייבות אימות בחשבון; הקובץ המקומי אינו ראיה שהוחלו אצל הספק.

79.3 **אימות שהושלם:** 16 בדיקות חסימה של התמונה ו־13 בדיקות SQL ב־AMD64 עברו ללא רשת. Hash הסכמה זהה לתוצאת ARM64. הנתיב המדויק ל־psql נבדק עם סביבת משתנים מצומצמת. [בדיקות המבצע](../../outputs/launch-validation-2026-09-09/postgres-bootstrap-runner-671f8183/runner-validation.json), [תצורה והשוואת ארכיטקטורות](../../outputs/launch-validation-2026-09-09/postgres-bootstrap-runner-671f8183/configuration-validation.json). סכמת Railway שנשמרה קודם אומתה; הורדה חדשה החזירה HTTP 403, והמסמכים הרשמיים נבדקו בנפרד. אין הוכחת חיבור TLS מאומת מהבדיקות המקומיות.

79.4 **שלב 3, יתרת הביצוע:** לזהות חשבון וסביבה → להכין מסד ותפקידי מיגרציה → להכין קובץ סיסמה ו־CA בזמן Runtime מחוץ ל־Image → לאמת TLS ו־Release → לפרוס את תיקיית context בשירות ייעודי ולהפעיל פעם אחת → לאמת סכמה והרשאות Runtime. אריזת המבצע המקומית מסעיף 78.4 הושלמה; תצורת הקבצים אצל הספק והביצוע החי עדיין פתוחים.

79.5 שלושה ניסיונות Vercel הסתיימו ללא הוכחת התחברות. נותרו **12 שלבים ראשיים**; אין אומדן כולל אמין לפני פתיחת גישת הספקים. ההכרעות R203–R204 מתועדות במסמך ההחלטות. אין בסבב זה הוכחת פריסה או QR חי.


## 80. קריאת הקובץ המקורי לאחר עריכת כיתוב — 11.09.2026

80.1 בזמן המתנה לאימות Railway קודם פער מוגדר של W09/W10: קובץ מדיה שכבר נשמר ונסרק נחסם אחרי עריכת כיתוב. בדיקת PostgreSQL שיחזרה ACCESS_DENIED לפני התיקון.

80.2 נוסף מסלול בדיקת מקור ייעודי לקריאת קובץ קיים, שמאפשר edited רק בסוג מדיה תואם וללא סתירה. Acquisition נשאר מחויב למקור שלא נערך. מקור, Binding, משימת Upload, Receipt, גרסת אובייקט וסריקה נשמרים; Membership, הסכמה, חיבור ומחיקה נבדקים מחדש לפני מסירת Bytes. גם כפתור הבקשה ב־Inbox הותאם, תוך שימור תצורת ההפעלה ואימות Clerk.

80.3 [דוח האימות והפרסום](../../outputs/launch-validation-2026-09-09/media-caption-file-validation.json), [החלטה R206](launch-decisions-2026-09-09.md#7202-r206--קריאת-הקובץ-המקורי-לאחר-עריכת-כיתוב-11092026). כישלון הרגרסיה הראשוני ותוצאות הכנת התרחישים נשמרים. אין שינוי בסכמה: נותרו 76 מיגרציות PostgreSQL. ה־Build/CI וחבילות ההפעלה הקודמות נשארים קשורים ל־671f8183 ואינם מוכיחים פרסום של התיקון החדש.

80.4 Vercel אומת והפרויקט הוגדר בסבב הקודם. קוד Railway הראשון פקע ללא אימות; לא בוצעו פריסה, שימוש בדפדפן או בדיקת QR. נותרו 12 שלבים ראשיים. אומדן הסבב המקומי המקורי 30–60 דקות מתייחס לתיקון הזה בלבד; זמן ההשקה הכולל עדיין אינו ידוע.


80.5 האימות המקומי הסופי עבר: 4,506 בדיקות כלליות, שתי הבניות וכל 206 בדיקות PostgreSQL של סוויטת היסטוריית המדיה, ללא כשל או דילוג. עברו גם TypeScript, Lint, Source/Interface guardrails וסריקת סודות. שבע בדיקות מסד חדשות כוללות שינוי הרשאה ומחיקה בזמן העברת גוף הקובץ; בדיקת הממשק מכסה הצעת בקשה אחרי עריכת כיתוב והסתרה במחיקה/סתירה. פרסום, CI מרוחק וקבלה חיה נשארים נפרדים לפי דוח האימות.


# 81. סיכום Dashboard מתוך דוח מורשה — 11.09.2026

81.1 W18 קיבל חיבור נתונים מקומי: נתיבי /workspace ו־/workspace/dashboard טוענים דוח דרך reports.read הקיים, לאחר הגנת הזהות. ארבעת הכרטיסים מציגים מדדים מוגדרים לפי R207, טווח תאריכים ושעת Snapshot, וקישור למסך הדוחות המפורט.

81.2 תקופת ברירת המחדל היא 30 תאריכים כולל היום ב־UTC, ולא החודש הקלנדרי. סיכום שיחות כולל גם שיחות סגורות שההודעה האחרונה בהן בטווח; סיכום קמפיינים כולל כל מצב לפי תאריך יצירה. המונחים מופיעים בממשק במפורש. אין חיבור שגוי של ספירת שיחות למספר אנשי קשר או של אירועי AI לעלות כספית.

81.3 התצוגה מותרת רק כשהדוח ready וקיים. מצב חסום או שגיאה מסתירים גם דוח קודם ומוצגים כהיעדר נתונים, לא כאפס. אין API, הרשאה או מיגרציה חדשים. הבדיקות משתמשות בתשתית דוח הפרוטוקול הקיימת, ללא נתוני הדגמה במוצר.

81.4 [דוח האימות והפרסום](../../outputs/launch-validation-2026-09-09/dashboard-report-validation.json) הוא מקור מצב הסבב. W18 עדיין דורש קבלת Staging מול נתונים אמיתיים ו־QA בדפדפן. 12 השלבים הראשיים נשארים פתוחים; אומדן ההשקה הכולל אינו ידוע. Railway עדיין דורש השלמת התחברות באתר, וקוד התחברות אינו הוכחת גישה.

# 82. השלמת תכנות לפני חיבור חשבונות — 11.09.2026

82.1 הוראת Tal משנה את קדימות העבודה: לממש את יתרת AI/Knowledge ו־Billing מקומית לפני חזרה לאשפי כניסה. העלאה, שליחה וגבייה אמיתיות נשארות כפופות לבדיקות השילוב. שלושת התחומים הם חבילות עבודה, לא שלוש משימות בודדות.

82.2 התחיל מימוש R208: מתאם Responses עם תצורה מפורשת ו־Rate card בתוקף, רשימת מודלים מותרת, בקשה חד־פעמית מוגבלת, כלי מודל כבויים, store=false, תשובה מובנית ובדיקת מזהי ציטוטים. נוסף רישום Usage לסירוב או תשובה שנדחתה כאשר הספק החזיר Usage תקף. מתאם HTTP אינו יומן עמיד ואינו מוכיח מניעת חיוב חוזר לאחר Restart.

82.3 אחזור מקומי דרך PostgreSQL מחליף את retriever החסום ב־Worker; הוא קורא עד 100 קטעים ממקורות ready השייכים לעסק ולמקורות שנבחרו, בודק Hash וזהות, ובוחר עד 20 קטעים בטווח 64 KiB. הציון הוא כיסוי מילות שאילתה, לא הסתברות נכונות ולא Eval סמנטי. נוסף מפענח UTF-8 ל־text/plain ו־text/markdown; PDF/מסמכי Office אינם נתמכים בו. הוא עדיין אינו מחובר למסלול העלאה חדש.

82.4 הפיתוח נשאר פתוח: חבילת S3/Scan/Upload, יומן Generation עמיד ושריון תקציב, חיבור מתאם Responses ל־Worker ולמוכנות ההפעלה, AI Sender לאחר אישור, Paddle Checkout/Webhooks/Entitlements/Reconciliation, וקבלת UI ומסירה. אין סימון של כל התכנות כמושלם. [מצב הבדיקות](../../outputs/launch-validation-2026-09-09/code-first-validation-20260911.json).


# 83. סגירת יומן Generation ושריון התקציב המקומי — 11.09.2026

83.1 R209 השלים מקומית את C2.1/C2.2: ספירת קלט Responses, שריון אטומי לפי תקציב שרתי, מיגרציה 0076 ליומן, Dispatch יחיד, שמירת תוצאה ו־Usage באותה עסקה, Replay ושמירת אי־ודאות לאחר כשל. Worker מחובר למתאם העמיד בלבד וזקוק להפעלה מפורשת.

83.2 עברו 4,540 בדיקות כלליות, 16 תרחישי PostgreSQL בשתי גרסאות נפרדות — 16.13 ו־17.11 — שתי הבניות, TypeScript, Source/Interface guardrails, Lint וסריקת סודות. Lint מחזיר אפס שגיאות ו־28 אזהרות קיימות. נוספה בדיקת CI; אין טענה שהיא כבר רצה מרחוק. [דוח R209](../../outputs/launch-validation-2026-09-09/ai-journal-validation-20260911.json).

83.3 כלל התכנות עדיין לא הושלם. נותרו 6 תתי־שלבים: אישור ושליחת AI, שני שלבי Knowledge, שני שלבי Paddle וביקורת/תפעול/Reconciliation. סדר העבודה והערכת הבסיס שנותרה, 50–100 שעות הנדסה בביטחון נמוך, מופיעים ב[תור השלמת הקוד](code-completion-plan-2026-09-11.md). 12 שלבים ראשיים נשארים פתוחים; החיבורים והקבלה החיה יבוצעו אחרי התכנות לפי הוראת Tal.


# 84. שליחת AI מאושרת ותיקון מכסות — 11.09.2026

84.1 R210 סוגר מקומית את C2.3: תור נפרד לכל אישור מאומת, בדיקה חוזרת לפני POST, מניעת שליחה חוזרת לאחר אי־ודאות, שמירת הודעת Inbox וסטטוס מסירה, והצגת מצבי המתנה/כשל. מספר המיגרציות הוא 78. תוקנו הגדרות הפעלת AI ונעילת העסק במנגנון המכסה המשותף.

84.2 ראיות מקומיות: 4,544 בדיקות כלליות; 27 בדיקות AI על כל אחת מגרסאות PostgreSQL 16.13/17.11; 13 בדיקות שליחה ידנית. שתי הבניות ובדיקות TypeScript/Source/Interface/Lint נכללות בדוח. אלה בדיקות מקומיות, ולא קבלת ספק או הוכחת מוכנות לשוק.

84.3 נותרו C1.1/C1.2, C3.1/C3.2 ו־C4.1 — חמישה תתי־שלבים, וסכום אומדני בסיס של 40–80 שעות הנדסה בביטחון נמוך. 12 השלבים הראשיים נשארים פתוחים. [דוח R210](../../outputs/launch-validation-2026-09-09/ai-delivery-validation-20260911.json), [הכרעות R210](launch-decisions-2026-09-09.md#7206-r210--שליחת-תשובת-ai-לאחר-אישור-ושריון-מכסה-תקין-11092026).

## 85. R211 — השלמת העלאת מקורות ידע מקומית, 11.09.2026

85.1 C1.1/C1.2 הושלמו מקומית: BFF/Railway/PostgreSQL/S3, תור עמיד, סריקה מוגנת לפי גרסה, חילוץ TXT/Markdown וטופס רענון מצב. גבול קובץ 128 KiB; PDF/Office מחוץ לגרסה הראשונה. אין הפעלה חיה או אישור GuardDuty מהחשבון.

85.2 סדר ההמשך המחייב: C3.1 תשלום וחתימת Webhook → C3.2 הרשאות מנוי וסדר אירועים → C4.1 ביקורת, בדיקות ותפעול תקלות → חיבור שירותים וקבלה חיה. נותרו 3 תתי־שלבים, בסיס 24–48 שעות הנדסה בביטחון נמוך, ו־12 שלבים ראשיים.

85.3 [תור השלמת קוד](code-completion-plan-2026-09-11.md), [הכרעות R211](launch-decisions-2026-09-09.md#7207-r211--מקורות-ידע-דרך-railways3-11092026), [הפעלה ומגבלות](../knowledge-ingestion-runtime.md).


# 86. R212 — Checkout ושיוך מנוי ב־Paddle, 11.09.2026

86.1 C3.1 מאומת מקומית: API/BFF לבעל העסק, עסקה אחת ביומן עמיד, Worker, חלון Paddle.js ו־Webhook חתום. העסק והמחיר אינם נבחרים מקלט חופשי בדפדפן. גרסה מאוחרת אינה מוחלפת באירוע ישן; אובדן תשובה אינו יוצר ניסיון חיוב נוסף.

86.2 C3.2 נשאר בביצוע: העובדות נשמרות אך זכאות בתשלום, Customer Portal, רכישה חוזרת ותיקון יצירה לא ודאית עדיין אינם ממומשים במלואם. C4.1 נשאר פתוח. **אין עדיין הכרזה שכל התכנות הסתיים או שניתן להפעיל חיוב ללקוחות.**

86.3 נותרו שתי חבילות פיתוח ובדיקות הכוללות עשר תתי־משימות בטבלת סעיף 7 של [תור הפיתוח](code-completion-plan-2026-09-11.md). אומדן הבסיס המצטבר: 16–32 שעות הנדסה, בביטחון נמוך. 12 שלבים ראשיים נותרים פתוחים, כולל חיבורים וקבלה חיה בסוף התכנות. [הוראות הפעלה ומקורות Paddle](../paddle-checkout-runtime.md).

## 87. R213 — אכיפת מנוי בזמן ביצוע, 11.09.2026

87.1 נסגרו מקומית C3.2.1/C3.2.2: מדיניות זכאות ואכיפה ב־API ובגבולות Worker. הבדיקות כוללות ביטול בזמן ספירת Tokens, לפני Seal של הודעה, ולפני העלאה, ושמירת Usage שכבר נוצר אחרי הביטול. מקור האמת הוא PostgreSQL; כיבוי Paddle אינו מבטל את מדיניות החיוב.

87.2 הוכרעו עצירה אחרי 15 דקות ללא אימות, ללא הארכת התקופה ששולמה וללא חסימת מסכי המידע והחיוב. זהו כלל Connect; לא SLA של Paddle. בדיקות מקומיות אינן אישור להפעלת חיוב ללקוחות. [מדיניות, תרחישים ומגבלות](../paddle-paid-access-runtime.md).

87.3 נותרו 8 תתי־משימות ב־2 חבילות לפי סעיף 8 בתור ההשלמה, ו־12 שלבים ראשיים. 16–32 שעות הן סכום אומדני בסיס בביטחון נמוך. אין קיצור אוטומטי של האומדן או הבטחת תאריך בשל מעבר בדיקות. [הכרעות R213](launch-decisions-2026-09-09.md#7209-r213--אכיפת-מנוי-בזמן-ביצוע-11092026).

## 88. R214 — ניהול מנוי, משלם משותף ורכישה חוזרת, 11.09.2026

88.1 נסגרו מקומית C3.2.3/C3.2.4. ניהול המנוי עובר לפורטל Paddle הציבורי עם אימות המשלם; Connect מציג את המנוי של העסק הנוכחי. ביטול סופי שאומת לאחרונה מאפשר רכישה חדשה עם היסטוריה ומניעת Replay בין דורות. תוצאות לא ודאיות ועסקאות שבוטלו לפני התשלום נשארות לבירור C3.2.5.

88.2 נותרו **6 תתי־משימות ב־2 חבילות קוד**. סדר מפורט ותנאי סיום נמצאים בסעיף 9 של [תוכנית השלמת הקוד](code-completion-plan-2026-09-11.md). אומדן מתוקן: **12–24 שעות הנדסה בביטחון נמוך**, ללא הבטחת תאריך. 12 שלבי ההשקה הראשיים נשארים פתוחים; חיבורי השירותים בסיום התכנות בהתאם להוראת Tal.

88.3 ראיות: 4,573 בדיקות כלליות, שתי הבניות, 24 תרחישי Paddle בכל PostgreSQL 16.13/17.11 ו־56 תרחישי Worker ב־17.11. אין חיוב, קבלת פורטל חיה או Deployment. [דוח R214](../../outputs/launch-validation-2026-09-09/paddle-lifecycle-validation-20260911.json), [הכרעות](launch-decisions-2026-09-09.md#7210-r214--ניהול-מנוי-ורכישה-חוזרת-11092026).

## 89. R215 — שחזור Checkout ותיקוני שילוב, 11.09.2026

89.1 נוספו Seal חד־פעמי, שמירת ראיות מתשובת היצירה, שחזור GET בלבד וסגירת ניסיון שלא נשלח או עסקה שביטולה אומת. לא נפתחים ניסיונות unknown ללא זהות מקורית, ולא מנקים סתירת מנוי אוטומטית. C3.2.5 עדיין פתוח. [הכרעות ומקורות](../paddle-checkout-recovery.md).

89.2 בודק הליבה הורחב לכל 83 המיגרציות וחשף נעילות חסרות שתוקנו ברכיבי Bot. הוא עדיין דורש Fixture עדכני להוכחת גבול ספק; C4.1.4 אינו סגור. נשמרות דרישות הסכמה והציפיות לתוצאה חיובית מלאה. 34 תרחישי Paddle עברו בכל PostgreSQL 16.13/17.11, ועוד 56 תרחישי Worker ב־16.13. אין פריסה או חיוב חי.

89.3 נותרו 6 תתי־משימות בשתי חבילות ו־12 שלבים ראשיים. אומדן 12–24 שעות הנדסה בביטחון נמוך; פירוט מעודכן בסעיף 10 של [תוכנית השלמת הקוד](code-completion-plan-2026-09-11.md). חיבורי ספקים, Pilot ו־Canary נשארים אחרי הקוד. [ראיות ומגבלות](../../outputs/launch-validation-2026-09-09/paddle-recovery-validation-20260911.json).

## 90. R216 — סגירת מוכנות לפרסום AI, 11.09.2026

90.1 C4.1.1 הושלם מקומית. דיווח Worker של עד 90 שניות, מחירון בתוקף, זכאות, USD, אישור נציג ומקורות מוכנים מחליפים את החסימה הקבועה. הבדיקות בפרסום רצות באותה טרנזקציה, כולל בדיקה חוזרת וביטול מלא במקרה כשל Audit. אין העברת מפתח ספק ל־API. [המימוש והגבולות](../ai-publication-readiness.md).

90.2 נותרו 5 תתי־משימות: C4.1.4, C3.2.5, C4.1.2, C4.1.3 ו־C4.1.5. אומדן 10–20 שעות הנדסה בביטחון נמוך ו־12 שלבים ראשיים. הבדיקה ההיסטורית נשארת פתוחה; הוקדם פער פרסום עצמאי. סדר ותנאי סיום בסעיף 11 של [התוכנית הפעילה](code-completion-plan-2026-09-11.md).

90.3 עברו 20 הרצות תרחיש פרסום בשתי גרסאות PostgreSQL ועוד 56 תרחישי רגרסיה ב־17.11, עם 84 מיגרציות. חיבורי שירותים, קבלה חיה, Pilot ו־Canary לא בוצעו. [דוח R216](../../outputs/launch-validation-2026-09-09/ai-readiness-validation-20260911.json).


## 91. R217 — הכרעות ושחזור Paddle פרטי, 11.09.2026

91.1 C3.2.5 נסגר מקומית. השלמה לפי ראיות תמיכה, סגירת יצירה שהוכחה כלא קיימת והכרעה בסתירת מנוי ממומשות בכלי פרטי, עם הרשאה פגת־תוקף לפי Login/עסק/סביבה, הכנה לחמש דקות, GET חוזר ו־Audit אטומי. אין POST נוסף, פתיחת מנוי שבוטל או ניקוי Review ישן באמצעות Replay. [תפעול](../paddle-operator-recovery.md).

91.2 נותרו 4 תתי־משימות: C4.1.4, C4.1.2, C4.1.3 ו־C4.1.5. אומדן 8–16 שעות הנדסה בביטחון נמוך. 12 שלבי ההשקה נשארים פתוחים; חיבורי שירותים וקבלה חיה מתבצעים אחרי הקוד. [התוכנית הפעילה](code-completion-plan-2026-09-11.md#12-r217--סגירת-שחזור-paddle-תפעולי-11092026).

91.3 47 תרחישי Paddle עברו בכל PostgreSQL 16.13/17.11, עם 85 מיגרציות, כולל משתמש מסד נפרד בעל הרשאות מוגבלות. [דוח R217](../../outputs/launch-validation-2026-09-09/paddle-operator-validation-20260911.json). חיבורים, פריסה וקבלת Paddle אמיתית לא בוצעו.


## 92. R218 — שמירה ומחיקת גרסאות Knowledge, 11.09.2026

92.1 C4.1.3 נסגר מקומית: זמן סגירה עמיד במסד, מדיניות מלאה ללא ברירת מחדל, בדיקת עיכוב משפטי, הרשאה פרטית שפגה, הוצאת מקור משימוש ויומן מחיקה אטומי. מחיקה לפי VersionId בלבד, ללא עקיפת Object Lock; כשל לא ודאי נשמר ומתיר לכל היותר שלושה ניסיונות לאותה גרסה לפני בירור חדש. [מימוש ותפעול](../knowledge-object-retention.md).

92.2 נותרו 3 תתי־משימות: C4.1.4, C4.1.2 ו־C4.1.5. אומדן 7–14 שעות הנדסה בביטחון נמוך; 12 שלבי ההשקה עדיין פתוחים. חיבורים וקבלה חיה בסיום הקוד. [התוכנית המפורטת, סדר הביצוע ותנאי הסיום](code-completion-plan-2026-09-11.md#13-r218--סגירת-retention-לקובצי-ידע-11092026).

92.3 עברו 4,588 בדיקות כלליות ושתי הבניות, ועוד 154 תרחישי PostgreSQL: 26 Knowledge בכל 16.13/17.11 ו־102 רגרסיות אחרות ב־17.11, על 86 מיגרציות. בודק הליבה ההיסטורי עדיין פתוח. [דוח אימות R218](../../outputs/launch-validation-2026-09-09/knowledge-retention-validation-20260911.json). לא נמחקו קבצי לקוחות; אין קבלת AWS חיה או פריסה. אין להסיק מחיקת כלל מידע העסק ממחיקת אובייקט S3.


## 93. R219 — מעבר ליבה מלא ושימור שדרוג, 12.09.2026

93.1 C4.1.4 מאומת מקומית: 86 מיגרציות, 101 תרחישים בכל PostgreSQL 16.13/17.11, בחלוקה ל־92 נוכחיים ו־9 היסטוריים. ראיות הפעולה והתוצאה ההיסטוריות שרדו את השדרוג. מסלול Bot עודכן להוכחת Credential/Permit/Session ונעילת עסק תוקנה באישורי מסירת קמפיין. [הוראות הבודק](../postgresql-core-verifier.md).

93.2 נותרו C4.1.2 ו־C4.1.5, באומדן 4–8 שעות הנדסה בביטחון נמוך. 12 שלבי השקה עדיין פתוחים; חיבורים וקבלה חיה לאחר הקוד. [התוכנית הפעילה](code-completion-plan-2026-09-11.md#14-r219--סגירת-בודק-postgresql-ותיקון-אישורי-מסירה-12092026), [דוח אימות](../../outputs/launch-validation-2026-09-09/core-current-protocol-validation-20260912.json). אין בקשות ספק, פריסה או קבלת שירות חי בסבב הזה.


## 94. R220 — בירור עלות, מסירה והעלאה, 12.09.2026

94.1 C4.1.2 נסגר מקומית. תיקוני עלות שומרים Usage מקורי, מסירה חיובית מקרינה הודעה אחת, הכרעה שלילית שומרת קליטת אישור מאוחר, ו־Knowledge חוזר לסריקה רק לפי VersionId ותוכן מאומתים. נמצא ותוקן גם המסלול החסר לבירור הודעה ידנית שחוסמת שיחה. [תפעול והרשאות](../uncertain-outcome-recovery.md).

94.2 נותר תת־שלב C4.1.5: ביקורת רוחבית, תיקון ממצאים וקבלת גרסת המסירה המקומית. טבלת W19/W20 עודכנה כדי שלא תמשיך לתאר קוד שכבר הושלם כחסר. פערי מדיה/סנכרון בתוכניות הקודמות אינם נמחקים; הם נבדקים במפורש בביקורת. אין הכרזת סיום כל התכנות. [תור ההמשך](code-completion-plan-2026-09-11.md#15-r220--סגירת-תוצאות-לא-ודאיות-12092026).

94.3 4,588 בדיקות כלליות ושתי הבניות עברו. ראיות 90 המיגרציות, PostgreSQL 16/17, משתמשי מסד מוגבלים ושדרוג נשמרות ב[דוח הסבב](../../outputs/launch-validation-2026-09-09/uncertain-recovery-validation-20260912.json). האומדן לביקורת הוא 2–4 שעות בביטחון נמוך; יתרת המימוש תעודכן אם תימצא. 12 שלבי ההשקה נשארים פתוחים, ללא מועד השקה מאומת.


## 95. R221 — סגירת ניקוי המדיה בקוד המקומי, 12.09.2026

95.1 A221-3/W10 ממומש: גרסאות לא מזוהות או מרובות נבדקות ונמחקות בנפרד, בהתאם למדיניות גיל/סגירה, Hold והרשאת Login. ניקוי אוטומטי מוגבל בוחר מועמדים, מאמת כל גרסה ושומר תור וראיות; הרשאות נבדקות שוב לפני בקשת ספק. כלי התפעול אינם API לקוחות. הוספו גם חסימות במסלול הניקוי הקודם כדי שלא יעקוף Hold או הוצאה משימוש. מקור, הודעות, אישורי העלאה וראיות סריקה נשמרים. [מדריך](../meta-media-retention.md).

95.2 פער הכיתוב A221-1 אומת בקוד הקיים. מדיניות A221-4 הוכרעה לאחר קריאת תיעוד Meta הרשמי: סיום דיווח הספק ועיבוד הנתונים שהגיעו אינם הוכחת שלמות אצלנו. A221-2 עדיין דורש מימוש חיבור מחדש עם מחזור נפרד ושימור הישן; זו אינה משימת חשבון בלבד. [ממצאים וראיות](../../outputs/launch-validation-2026-09-09/media-retention-validation-20260912.json).

95.3 נותרו שני תתי־שלבים בתוך C4.1.5: חיבור מחדש (6–12 שעות) ומסירה/רגרסיה (2–4 שעות), **8–16 שעות הנדסה בביטחון נמוך**. 12 שלבי ההשקה נשארים פתוחים, ללא תאריך מאומת. חיבורים, פריסה וקבלה חיה יבוצעו לאחר תור הקוד לפי הוראת Tal.


# 96. R222 — התקדמות בחיבור מחדש, 12.09.2026

96.1 מחזורי בקשות נפרדים, עדות ניתוק, שימור תוצאות מאוחרות ויומן קליטה ללא שיוך מומשו. רגרסיית הנעילות תוקנה, בלי שינוי בראיות הישנות או Replay לבקשות ספק. מצב הייבוא בבירור מוצג בשלוש השפות.

96.2 הסיום המחייב הוא [סעיף 17 בתור השלמת הקוד](code-completion-plan-2026-09-11.md#17-r222--מחזורים-נפרדים-ושימור-קליטה-ללא-שיוך-12092026): שיוך/הצגת היסטוריה מורשים בין מחזורים, ואז מסירה סופית. שני תתי־שלבים, 8–16 שעות הנדסה בביטחון נמוך. 12 שלבי ההשקה נשארים פתוחים; אין תאריך השקה מאומת.

# 97. R223 — היסטוריה קיימת אחרי Signup חדש, 12.09.2026

97.1 הושלמו הרשאות קריאה להיסטוריה קיימת ולגרסת קובץ שכבר נשמרה ונסרקה. החיבור העדכני חייב להתייחס לאותו עסק, WABA ומספר; Signup ו־Receipt חייבים לאמת את גרסתו. המקור נשאר במחזור המקורי, ומסלולי רכישה, שיגור ועיבוד אינם נפתחים דרך הרשאת הקריאה. סירוב שיתוף, סתירה, חסימת עסק, חברות לא פעילה, סריקה חוסמת והוצאה משימוש ממשיכים לחסום.

97.2 יתרת הביצוע המחייבת היא [סעיף 18 בתור השלמת הקוד](code-completion-plan-2026-09-11.md#18-r223--קריאת-היסטוריה-לאחר-חיבור-מחדש-12092026): שיוך וייבוא אצוות למחזור מוכח, ואז ביקורת מסירה. **2 תתי־שלבים, 6–12 שעות הנדסה בביטחון נמוך; 12 שלבי השקה פתוחים**. זו אינה הכרזת השלמת תכנות או קבלה חיה. [ראיות R223](../../outputs/launch-validation-2026-09-09/history-reads-validation-20260912.json).

# 98. R224 — סגירת תור התכנות המקומי, 12.09.2026

98.1 A221-2 הושלם: מקור היסטוריה וקבצים נשמר במחזור המקורי; אצווה משויכת רק בידי Login תפעולי מורשה, מול בקשת מקור שהתקבלה וראיה חיצונית מדויקת. ייבוא אטומי מונע כפילות בין מחזורים ומכבד סירוב. בוצעו שדרוג נתונים ובדיקות מקביליות, הרשאות וכשל. הכלי אינו משיג או מאמת עצמאית ראיית ספק; בהיעדרה האירוע נשאר בבירור.

98.2 הושלמה גם ביקורת המסירה C4.1.5.3 בתכולה המוגדרת: מפת התהליכים, שתי הבניות, 4,598 בדיקות כלליות, 426 תרחישי WhatsApp/מדיה בכל PostgreSQL 16.13 ו־17.11, 101 תרחישי בודק הליבה בכל גרסה ו־149 תרחישי מוצר נוספים על 17.11. סריקת סודות ושימור קובצי הבסיס נדרשים בקיבוע המקומי המתועד בדוח. [מסירת קוד](local-code-delivery-2026-09-12.md), [ראיות R224](../../outputs/launch-validation-2026-09-09/history-import-validation-20260912.json).

98.3 **0 תתי־שלבי קוד ידועים לפני החיבורים; 12 שלבי השקה עדיין פתוחים.** האומדן ליתרת תור הקוד הוא 0 שעות. משך החיבורים, הקבלה ותיקונים שיתגלו אינו ידוע; אין תאריך השקה מאומת. התוכנית המלאה, זמני ה־Pilot וה־Canary והחרגות התכולה נשמרים. [סדר חיבורים מרוכז](service-connections-handoff-2026-09-12.md), [פירוט סגירת התור](code-completion-plan-2026-09-11.md#19-r224--ייבוא-משויך-וביקורת-מסירה-מקומית-12092026).
