...
export function PaddleBillingPanel({ language }: { language: InterfaceLanguage }) {
  ...
  const status = !result ? m.loading : !view ? result.status === "configuration-required" ? m.config : result.status === "permission-denied" || result.status === "unauthenticated" ? m.denied : m.failed :
    state === "queued" || state === "creating" ? m.queued : state === "unknown" ? m.unknown : state === "ready" ? m.ready : state === "completed" ? m.completed : state === "rejected" ? m.rejected : state === "closed" ? m.closed : m.idle;
  return <section className="card billing-card paddle-billing-panel">
    ...
    {view?.subscription && <div><p>{m.statuses[view.subscription.status]}</p><p>{m.reference} <bdi>{view.subscription.id}</bdi></p>{view.subscription.scheduledChange && <p>{m.scheduled[view.subscription.scheduledChange.action]} <time dateTime={view.subscription.scheduledChange.effectiveAt}>{new Intl.DateTimeFormat(language, { dateStyle: "medium", timeStyle: "short", timeZone: "UTC" }).format(new Date(view.subscription.scheduledChange.effectiveAt))} UTC</time></p>}{view.subscription.endsAt && <p>{m.ends} <time dateTime={view.subscription.endsAt}>{view.subscription.endsAt.slice(0, 10)}</time></p>}{view.subscription.needsReview && <p>{m.review}</p>}</div>}
    {view?.canManage && view.subscription && <div><a className="secondary-button" href={view.customerPortalUrl} target="_blank" rel="noopener noreferrer">{m.portal}</a><p>{m.portalHelp}</p></div>}
    <div className="billing-logic">
      {view?.canManage && view.canCreateCheckout && <button className="primary-button" disabled={pending} onClick={() => run(async () => setResult(await createPaddleCheckoutAction(view.attempt)))}>{view.attempt === 0 ? m.create : m.repurchase}</button>}
