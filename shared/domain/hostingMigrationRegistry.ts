...
export const HOSTING_MIGRATION_REGISTRY = Object.freeze([
  ...
  capability({
    ...
    cutoverBlocker:
      "The authenticated runtime, initial reads, guarded contacts.save mutation, provider-neutral PostgreSQL transaction executor, node-postgres transaction adapter, pool contract, PostgreSQL contacts.list read, a full grant/unsubscribe Vercel BFF over immutable contact-consent evidence with atomic latest-event projection, and four contact-organization mutations with atomic receipts, audit, replay snapshots, and no D1 fallback exist; tenant-isolated campaign-audience reads, atomic contact-import, conversation/message inbox persistence, bot-flow version/publication, reply-delivery and bot-runtime continuation/handoff persistence, knowledge-source lifecycle and atomic knowledge-passage persistence, AI-agent draft/version/source-link/publication persistence, serialized AI-runtime cost authorization/usage plus audited handoff persistence, AI reply approval-outbox persistence, tenant-subscription lifecycle persistence with immutable audit events, first-owner tenant provisioning with identity-conflict protection, registered production-decision persistence with immutable audit evidence, bounded system-admin tenant directory and audited profile updates, Meta connection/webhook and encrypted credential paths, immutable WhatsApp delivery-policy evidence and kill switch, serialized WhatsApp reservation/settlement/provider-cooldown ledger, provider-bound rolling phone-throughput enforcement, fenced worker-scheduler lease, message-template lifecycle, campaign-snapshot, campaign-dispatch and atomic provider-reconciliation persistence, single-statement reports.read adapter, forty-adapter PostgreSQL foundation, owned PostgreSQL API runtime composition, shared PostgreSQL token-bucket contracts for tenant mutations, system-admin mutations and Meta webhook ingress, bounded Node HTTP adapter, database readiness probe, ordered HTTP-before-pool shutdown owner, strict PORT configuration, SIGINT/SIGTERM lifecycle, and an executable API bootstrap also exist. All six operational-report source families are migrated, the complete authenticated reports.read HTTP path passed against PostgreSQL 16.13, and a deterministic registry maps all 36 D1 migrations and all 51 D1 tables to the 26 PostgreSQL migrations. Local rehearsals proved data migration and semantic parity for all ten slices and all 51 tables. A read-only single-transaction full-source snapshot boundary validates the exact 51-table D1 export contract without exposing its path or rows. A signed all-slice bundle executes the ten plans atomically and writes an immutable source-level replay receipt. A two-phase operator cutover requires an approved source digest and explicit confirmation before target access; its empty-source rehearsal passed against PostgreSQL 16.13. Liveness, PostgreSQL readiness and graceful SIGTERM shutdown are also proven locally. Cutover remains blocked by live Railway service configuration and remaining route wiring, provider-bound pool and rate-limit policy values, a live controlled-environment export and signed full rehearsal, remaining domain mutations, live account configuration, and staging evidence.",
  }),
  capability({
